// src/component/WashingMachine.jsx
...
const FILTER_CONFIG = {
  brand: ["Samsung", "LG", "Whirlpool", "Godrej", "Haier", "Panasonic", "Bosch", "Siemens", "IFB", "Electrolux", "Voltas Beko", "Hitachi", "Sharp", "Kenstar", "Croma", "Midea"],
  type: ["Front Load", "Top Load", "Semi Automatic", "Fully Automatic", "Washer Dryer Combo", "Portable Washer"],
  capacity: ["5kg", "6kg", "6.5kg", "7kg", "7.5kg", "8kg", "8.5kg", "9kg", "9.5kg", "10kg", "11kg", "12kg", "13kg", "14kg", "15kg+"],
  color: ["White", "Silver", "Black", "Gray", "Blue", "Red", "Gold", "Stainless Steel", "Inox", "Platinum"],
  washTechnology: ["Twin Tub", "Pulsator", "Fuzzy Logic", "Inverter Technology", "Direct Drive", "Eco Bubble", "6th Sense", "Steam Wash", "Auto Restart", "Smart Diagnosis"],
  spinSpeed: ["600 RPM", "800 RPM", "1000 RPM", "1200 RPM", "1400 RPM", "1600 RPM", "1800 RPM", "2000 RPM"],
  features: ["Heater", "Child Lock", "Delay Start", "Quick Wash", "Soak Feature", "Rinse Hold", "Auto Balance", "Foam Control", "Drum Clean", "Smart Connect", "WiFi Enabled", "Tub Clean", "Soft Closing Door", "LED Display", "Digital Display"]
};
...
function WashingMachine() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], capacity: [], color: [], washTechnology: [], spinSpeed: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, capacity: true, color: false, washTechnology: false, spinSpeed: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], capacity: [], color: [], washTechnology: [], spinSpeed: [], features: [] });
    setMaxPrice(100000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "washing machine" ||
    p.subCategory?.toLowerCase() === "washing-machine" ||
    p.name?.toLowerCase().includes("washing machine") ||
    p.name?.toLowerCase().includes("washer") ||
    p.name?.toLowerCase().includes("laundry")
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.washTechnology.length > 0 && !filters.washTechnology.includes(product.washTechnology)) return false;
    if (filters.spinSpeed.length > 0 && !filters.spinSpeed.includes(product.spinSpeed)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Wash Technology" sectionKey="washTechnology" items={FILTER_CONFIG.washTechnology} />
      <AccordionSection title=" Spin Speed" sectionKey="spinSpeed" items={FILTER_CONFIG.spinSpeed} />
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={150000} step={5000}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Washing Machines</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Washing Machines</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Washing Machines Found</h3>
