...
const FILTER_CONFIG = {
  brand: ["Portronics", "Zebronics", "Ambrane", "Syska", "Orient", "Bajaj", "Usha", "Havells", "Crompton", "Luminous", "Oakter", "Mi", "Realme", "OnePlus", "boAt", "Samsung", "Generic"],
  type: ["USB Fan", "Rechargeable Fan", "Handheld Fan", "Desktop Fan", "Clip Fan", "Neck Fan", "Stroller Fan", "Table Fan", "Foldable Fan", "Stand Fan"],
  powerSource: ["USB Powered", "Rechargeable Battery", "AA Battery", "AAA Battery", "Solar Powered", "AC Powered"],
  batteryLife: ["1-2 Hours", "2-3 Hours", "3-4 Hours", "4-5 Hours", "5-6 Hours", "6-8 Hours", "8-10 Hours", "10-12 Hours", "12-15 Hours", "15+ Hours"],
  speedSettings: ["Single Speed", "2 Speeds", "3 Speeds", "4 Speeds", "Variable Speed"],
  color: ["White", "Black", "Blue", "Pink", "Green", "Red", "Yellow", "Purple", "Orange", "Multicolor", "Silver", "Teal"],
  size: ["Mini (2-3 inch)", "Small (3-4 inch)", "Medium (4-5 inch)", "Large (5-6 inch)", "Extra Large (6-8 inch)"],
  features: ["Silent Operation", "LED Light", "Power Bank", "Digital Display", "Remote Control", "Adjustable Stand", "Foldable", "Carry Hook", "Bladeless", "Misting", "Ionizer", "Timer", "Auto Oscillation", "Night Light"]
};
...
function MiniFans() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], powerSource: [], batteryLife: [], speedSettings: [], color: [], size: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, powerSource: false, batteryLife: false, speedSettings: false, color: false, size: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], powerSource: [], batteryLife: [], speedSettings: [], color: [], size: [], features: [] });
    setMaxPrice(3000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "mini fan" ||
    p.category?.toLowerCase() === "portable fan" ||
    p.subCategory?.toLowerCase() === "mini-fan" ||
    p.name?.toLowerCase().includes("mini fan") ||
    p.name?.toLowerCase().includes("portable fan") ||
    p.name?.toLowerCase().includes("usb fan") ||
    p.name?.toLowerCase().includes("handheld fan") ||
    p.name?.toLowerCase().includes("rechargeable fan")
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Mini Fans</h1>
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Mini Fans</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Mini Fans</h2>
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-black/60" onClick={() => setMobileFiltersOpen(false)}>
          <div className="absolute top-0 left-0 w-[85%] h-full bg-white overflow-y-auto p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sticky bottom-0 bg-white pt-4 pb-2 mt-4 border-t">
              <button onClick={() => setMobileFiltersOpen(false)} className="w-full bg-green-600 text-white py-3 rounded-lg font-semibold">
                Apply Filters
              </button>
            </div>
          </div>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Mini Fans Found</h3>
