// src/component/Refrigerator.jsx
...
const FILTER_CONFIG = {
  brand: ["Samsung", "LG", "Whirlpool", "Godrej", "Haier", "Panasonic", "Bosch", "Siemens", "Hitachi", "Daewoo", "Electrolux", "Voltas Beko", "Sharp", "Hisense", "Toshiba", "Kenstar"],
  type: ["Single Door", "Double Door", "Side by Side", "French Door", "Mini Refrigerator", "Bottom Freezer", "Top Freezer", "Convertible Refrigerator", "Smart Refrigerator"],
  capacity: ["100L-150L", "150L-200L", "200L-250L", "250L-300L", "300L-350L", "350L-400L", "400L-450L", "450L-500L", "500L-600L", "600L+"],
  color: ["Black", "White", "Silver", "Steel", "Black Steel", "Stainless Steel", "Red", "Blue", "Gray", "Gold", "Brown"],
  energyRating: ["1 Star", "2 Star", "3 Star", "4 Star", "5 Star"],
  frostType: ["Direct Cool", "Frost Free", "Auto Defrost", "Manual Defrost"],
  features: ["Inverter Compressor", "Digital Inverter", "Smart Inverter", "Convertible", "Door Cooling", "Deodorizer", "Toughened Glass", "LED Display", "Ice Maker", "Water Dispenser", "WiFi Enabled", "Smart Diagnosis", "Anti-Bacterial Gasket"]
};
...
function Refrigerator() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], capacity: [], color: [], energyRating: [], frostType: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, capacity: true, color: false, energyRating: false, frostType: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], capacity: [], color: [], energyRating: [], frostType: [], features: [] });
    setMaxPrice(150000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "refrigerator" ||
    p.subCategory?.toLowerCase() === "refrigerator" ||
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.energyRating.length > 0 && !filters.energyRating.includes(product.energyRating)) return false;
    if (filters.frostType.length > 0 && !filters.frostType.includes(product.frostType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Energy Rating" sectionKey="energyRating" items={FILTER_CONFIG.energyRating} />
      <AccordionSection title=" Frost Type" sectionKey="frostType" items={FILTER_CONFIG.frostType} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Refrigerators</h1>
      
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Refrigerators</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Refrigerators</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Refrigerators Found</h3>
