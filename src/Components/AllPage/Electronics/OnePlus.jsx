...
const FILTER_CONFIG = {
  ram: ["6GB", "8GB", "12GB", "16GB", "24GB"],
  storage: ["128GB", "256GB", "512GB", "1TB"],
  color: ["Black", "White", "Silver", "Gold", "Blue", "Green", "Red", "Purple", "Matte Black", "Glacier Blue"],
  camera: ["48MP", "50MP", "64MP", "108MP", "200MP"],
  battery: ["4500mAh", "5000mAh", "5400mAh", "6000mAh"],
  display: ["6.5 inch", "6.7 inch", "6.8 inch", "6.9 inch", "7 inch"],
...
function OnePlus() {
  ...
  const [filters, setFilters] = useState({ ram: [], storage: [], color: [], camera: [], battery: [], display: [] });
  const [openSections, setOpenSections] = useState({ 
    ram: true, storage: true, color: true, camera: false, 
    battery: false, display: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ ram: [], storage: [], color: [], camera: [], battery: [], display: [], processor: [] });
    setMaxPrice(100000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.brand?.toLowerCase() === "oneplus" ||
    p.brand?.toLowerCase() === "one plus" ||
    p.name?.toLowerCase().includes("oneplus") ||
    p.name?.toLowerCase().includes("one plus")
  );
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Display" sectionKey="display" items={FILTER_CONFIG.display} />
      
      <div className="mb-4 pt-2">
        <h2 className="font-bold text-sm mb-2"> Max Price: ₹{maxPrice}</h2>
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={120000} step={1000}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-red-600 to-orange-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> OnePlus Mobiles</h1>
        <p className="mt-2 text-white/80">Never Settle - Flagship Killers with Premium Performance</p>
      
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">OnePlus Mobiles</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">OnePlus Mobiles</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No OnePlus Products Found</h3>
