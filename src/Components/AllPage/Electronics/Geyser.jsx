...
const FILTER_CONFIG = {
  geyserType: [
    "Storage Geyser", "Instant Geyser", "Gas Geyser", 
    "Tankless Geyser", "Heat Pump Geyser", "Solar Geyser",
    "Vertical Geyser", "Horizontal Geyser", "Wall Mounted"
  ],
  capacity: [
    "3 Litres", "5 Litres", "6 Litres", "10 Litres", 
    "15 Litres", "25 Litres", "35 Litres", "50 Litres",
    "75 Litres", "100 Litres", "150 Litres", "200 Litres"
  ],
  material: [
    "Stainless Steel", "Glass Lined", "Copper", "Enamel Coated",
    "PEX Lining", "Titanium", "Ceramic", "ABS Plastic Body"
  ],
  features: [
    "Energy Efficient", "5 Star Rating", "4 Star Rating", "3 Star Rating",
    "Fast Heating", "Thermostat Control", "Safety Valve", "LED Display",
    "Remote Control", "WiFi Enabled", "Smart Control", "Child Lock",
    "Anti Rust", "Corrosion Resistant", "Overheat Protection", "Auto Shutoff"
  ],
  brands: [
    "Bajaj", "Havells", "Crompton", "V-Guard", "AO Smith",
    "Whirlpool", "LG", "Samsung", "Orient", "Usha",
    "Butterfly", "Kenstar", "Racold", "Amerisan", "Eureka Forbes"
  ]
};
...
function Geyser() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category === "Geyser" || p.subCategory === "Geyser" || p.name?.toLowerCase().includes("geyser") || p.name?.toLowerCase().includes("water heater")
  );

  filteredProducts = filteredProducts.filter((product) => {
    if (product.offerPrice > maxPrice) return false;
    if (filters.geyserType.length > 0 && !filters.geyserType.includes(product.geyserType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title="Geyser Type" sectionKey="geyserType" items={FILTER_CONFIG.geyserType} />
      ...
      <div className="mb-4 pt-2">
        ...
        <div className="flex justify-between text-[10px] text-gray-400 mt-1">
          <span>₹0</span><span>₹6.2k</span><span>₹12.5k</span><span>₹18.7k</span><span>₹25k</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-red-600 to-orange-600 text-white text-center py-8">
        <h1 className="text-3xl font-bold">Water Geysers</h1>
        <p className="mt-2 text-white/80 text-sm">Hot Water - Always On Demand</p>
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Geysers</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7"> Geysers</h2>
          ...
          <div className="flex-1">
            <div className="bg-white rounded-xl shadow-sm p-3 mb-4 flex justify-between items-center flex-wrap gap-2">
              <div className="text-xs text-gray-500">
                Showing <span className="font-semibold text-gray-700">{filteredProducts.length}</span> geysers
              </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                <p className="text-gray-500 text-sm">No Geysers found matching your filters.</p>
                <button 
