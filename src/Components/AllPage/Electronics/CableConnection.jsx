...
const FILTER_CONFIG = {
  brand: ["Samsung", "Apple", "OnePlus", "Xiaomi", "Realme", "boAt", "Portronics", "Ambrane", "Syska", "Zebronics", "Mi", "Motorola", "Google", "Anker", "Belkin", "Spigen", "Amazon Basics"],
  cableType: ["USB-C to USB-C", "USB-C to USB-A", "Lightning Cable", "Micro USB Cable", "USB-C to Lightning", "USB-A to USB-A", "USB Extension Cable", "Type-C to Type-C", "Braided Cable", "Nylon Cable", "Flat Cable"],
  length: ["0.25m", "0.5m", "1m", "1.5m", "2m", "3m", "5m", "10m", "15m", "20m"],
  color: ["Black", "White", "Blue", "Red", "Green", "Silver", "Gray", "Purple", "Orange", "Multicolor"],
  chargingSpeed: ["Standard (5W)", "Fast (18W)", "Fast (20W)", "Fast (25W)", "Fast (33W)", "Fast (45W)", "Fast (65W)", "Fast (100W)", "Fast (120W)", "Super Fast", "Quick Charge", "Power Delivery"],
  material: ["PVC", "Nylon Braided", "TPE", "Silicone", "Fabric", "Aluminum Alloy", "Copper Core", "Kevlar"],
  compatibility: ["Android", "iPhone", "Tablets", "Power Banks", "Laptops", "Gaming Devices", "Cameras", "All Devices"],
  features: ["Fast Charging", "Data Sync", "Tangle Free", "Durable", "Heat Resistant", "Shielded", "Reinforced Joints", "LED Light"]
};
...
function CableConnection() {
  ...
  const [filters, setFilters] = useState({ brand: [], cableType: [], length: [], color: [], chargingSpeed: [], material: [], compatibility: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, cableType: true, length: false, color: false, chargingSpeed: true, material: false, compatibility: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], cableType: [], length: [], color: [], chargingSpeed: [], material: [], compatibility: [], features: [] });
    setMaxPrice(3000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "cable" ||
    p.subCategory?.toLowerCase() === "cable" ||
    p.name?.toLowerCase().includes("cable") ||
    p.name?.toLowerCase().includes("charging cable") ||
    p.name?.toLowerCase().includes("usb cable")
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.chargingSpeed.length > 0 && !filters.chargingSpeed.includes(product.chargingSpeed)) return false;
    if (filters.material.length > 0 && !filters.material.includes(product.material)) return false;
    if (filters.compatibility.length > 0 && !filters.compatibility.includes(product.compatibility)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Charging Speed" sectionKey="chargingSpeed" items={FILTER_CONFIG.chargingSpeed} />
      <AccordionSection title=" Material" sectionKey="material" items={FILTER_CONFIG.material} />
      <AccordionSection title=" Compatibility" sectionKey="compatibility" items={FILTER_CONFIG.compatibility} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-orange-600 to-red-600 text-white text-center py-12">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Cables Connection</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Cables Connection</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Cables Found</h3>
