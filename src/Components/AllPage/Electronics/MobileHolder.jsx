// src/component/MobileHolder.jsx
...
const FILTER_CONFIG = {
  brand: ["Portronics", "Zebronics", "Ambrane", "Syska", "boAt", "Mi", "Samsung", "OnePlus", "Apple", "Spigen", "Ringke", "ESR", "Gravity", "Fedo", "WizGear", "Lamicall", "UGreen"],
  type: ["Car Mount", "Desktop Stand", "Bike Mount", "Tripod Mount", "Magnetic Holder", "Clamp Holder", "Vent Clip", "Suction Cup", "Wireless Charger Stand", "Adjustable Stand", "Foldable Stand", "Ring Holder", "Grip Holder", "Universal Holder"],
  mountingType: ["Dashboard Mount", "Windshield Mount", "Air Vent Mount", "CD Slot Mount", "Desk Mount", "Tripod Mount", "Sticky Pad", "Magnetic Base", "Clip On"],
  color: ["Black", "White", "Silver", "Gray", "Blue", "Red", "Green", "Rose Gold", "Gold", "Space Gray"],
  compatibility: ["Universal", "Smartphones", "Tablets", "Phones with Case", "MagSafe Compatible", "All Android", "All iOS", "4-7 inch", "7-10 inch"],
  material: ["Plastic", "ABS Plastic", "Aluminum", "Metal Alloy", "Silicone", "Rubber", "Polycarbonate", "Carbon Fiber", "Glass"],
  features: ["360 Rotation", "Tilt Adjustment", "Screw-less Design", "One Touch Release", "Non-Slip Pad", "Soft Grip", "Wireless Charging", "Magnetic", "Foldable", "Compact", "Sturdy Build", "Heavy Duty"]
};
...
function MobileHolder() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], mountingType: [], color: [], compatibility: [], material: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, mountingType: false, color: false, compatibility: false, material: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], mountingType: [], color: [], compatibility: [], material: [], features: [] });
    setMaxPrice(3000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "mobile holder" ||
    p.subCategory?.toLowerCase() === "mobile-holder" ||
    p.name?.toLowerCase().includes("mobile holder") ||
    p.name?.toLowerCase().includes("phone holder") ||
    p.name?.toLowerCase().includes("car mount") ||
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold">Mobile Holders</h1>
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Mobile Holders</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Mobile Holders</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Mobile Holders Found</h3>
