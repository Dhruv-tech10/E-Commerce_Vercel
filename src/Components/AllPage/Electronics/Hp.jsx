...
const FILTER_CONFIG = {
  ...
  display: ["14 inch", "15.6 inch", "16 inch", "17.3 inch"],
  graphics: ["Intel Integrated", "NVIDIA GeForce GTX", "NVIDIA GeForce RTX", "AMD Radeon"],
  color: ["Black", "Silver", "Gray", "Gold", "White", "Blue"]
};
...
function Hp() {
  ...
  let filteredProducts = products.filter(p => 
    ...
    p.name?.toLowerCase().includes("pavilion") ||
    p.name?.toLowerCase().includes("envy") ||
    p.name?.toLowerCase().includes("spectre") ||
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-900 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> HP Laptops</h1>
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">HP Laptops</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">HP Laptops</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No HP Laptops Found</h3>
