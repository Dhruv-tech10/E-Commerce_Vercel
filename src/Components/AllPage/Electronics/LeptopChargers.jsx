// src/component/LaptopCharger.jsx
...
const FILTER_CONFIG = {
  brand: ["Dell", "HP", "Lenovo", "Asus", "Apple", "Acer", "Microsoft", "Samsung", "LG", "MSI", "Alienware", "Razer", "Portronics", "Ambrane", "Syska", "Zebronics", "boAt"],
  chargerType: ["Original OEM", "Compatible Replacement", "Universal Adapter", "GaN Charger", "USB-C Charger", "DC Barrel Charger", "Magsafe Charger", "Slim Adapter", "Travel Charger"],
  powerOutput: ["30W", "45W", "65W", "90W", "100W", "120W", "135W", "150W", "180W", "200W", "230W", "240W", "300W"],
  connectorType: ["USB-C", "Barrel Connector", "Magsafe 1", "Magsafe 2", "Magsafe 3", "Slim Tip", "Power Pin", "USB-C PD", "Universal Tip"],
  color: ["Black", "White", "Silver", "Gray", "Space Gray", "Gold"],
  cableLength: ["1m", "1.2m", "1.5m", "1.8m", "2m", "2.5m", "3m"],
  inputVoltage: ["100-240V", "110-240V", "120-240V", "Universal Voltage"],
  features: ["Fast Charging", "Overcharge Protection", "Short Circuit Protection", "Temperature Control", "Surge Protection", "LED Indicator", "Detachable Cable", "Foldable Plug", "Braided Cable"]
};
...
function LaptopCharger() {
  ...
  const [filters, setFilters] = useState({ brand: [], chargerType: [], powerOutput: [], connectorType: [], color: [], cableLength: [], inputVoltage: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, chargerType: true, powerOutput: true, connectorType: false, color: false, cableLength: false, inputVoltage: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], chargerType: [], powerOutput: [], connectorType: [], color: [], cableLength: [], inputVoltage: [], features: [] });
    setMaxPrice(8000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "laptop charger" ||
    p.subCategory?.toLowerCase() === "laptop-charger" ||
    p.name?.toLowerCase().includes("laptop charger") ||
    p.name?.toLowerCase().includes("adapter") ||
    (p.name?.toLowerCase().includes("charger") && p.name?.toLowerCase().includes("laptop"))
  );

  filteredProducts = filteredProducts.filter((product) => {
    if (product.offerPrice > maxPrice) return false;
    if (filters.brand.length > 0 && !filters.brand.includes(product.brand)) return false;
    if (filters.chargerType.length > 0 && !filters.chargerType.includes(product.chargerType)) return false;
    if (filters.powerOutput.length > 0 && !filters.powerOutput.includes(product.powerOutput)) return false;
    if (filters.connectorType.length > 0 && !filters.connectorType.includes(product.connectorType)) return false;
    if (filters.color.length > 0 && !filters.color.includes(product.color)) return false;
    if (filters.cableLength.length > 0 && !filters.cableLength.includes(product.cableLength)) return false;
    if (filters.inputVoltage.length > 0 && !filters.inputVoltage.includes(product.inputVoltage)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Charger Type" sectionKey="chargerType" items={FILTER_CONFIG.chargerType} />
      <AccordionSection title=" Power Output" sectionKey="powerOutput" items={FILTER_CONFIG.powerOutput} />
      <AccordionSection title=" Connector Type" sectionKey="connectorType" items={FILTER_CONFIG.connectorType} />
      <AccordionSection title=" Color" sectionKey="color" items={FILTER_CONFIG.color} />
      <AccordionSection title=" Cable Length" sectionKey="cableLength" items={FILTER_CONFIG.cableLength} />
      <AccordionSection title=" Input Voltage" sectionKey="inputVoltage" items={FILTER_CONFIG.inputVoltage} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold">Laptop Chargers</h1>

      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Laptop Chargers</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Laptop Chargers</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Laptop Chargers Found</h3>
