...
const FILTER_CONFIG = {
  ...
  color: ["Black", "Blue", "Green", "Red", "Silver", "Gold", "White", "Purple"],
  camera: ["8MP", "13MP", "16MP", "48MP", "50MP", "64MP"],
  battery: ["3000mAh", "3500mAh", "4000mAh", "4500mAh", "5000mAh", "6000mAh"]
};
...
function Micromax() {
  ...
  let filteredProducts = products.filter(p => 
    p.brand?.toLowerCase() === "micromax" ||
    p.name?.toLowerCase().includes("micromax")
  );
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={40000} step={500}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-teal-600 to-emerald-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Micromax Mobiles</h1>
       
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Micromax Mobiles</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Micromax Mobiles</h2>
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-black/60" onClick={() => setMobileFiltersOpen(false)}>
          <div className="absolute top-0 left-0 w-[85%] h-full bg-white overflow-y-auto p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sticky bottom-0 bg-white pt-4 pb-2 mt-4 border-t">
              <button onClick={() => setMobileFiltersOpen(false)} className="w-full bg-teal-600 text-white py-3 rounded-lg font-semibold">
                Apply Filters
              </button>
            </div>
          </div>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Micromax Products Found</h3>
