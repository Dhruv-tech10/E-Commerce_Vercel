...
const FILTER_CONFIG = {
  ...
  color: ["Black", "White", "Silver", "Gold", "Rose Gold", "Green", "Blue", "Purple", "Red"],
  camera: ["12MP", "24MP", "36MP", "48MP", "108MP"],
  battery: ["3000mAh", "3500mAh", "4000mAh", "4500mAh", "5000mAh"]
};
...
function IPhone() {
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-gray-700 transition">
        <span>{title}</span>
          ...
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
              <input type="checkbox" checked={filters[sectionKey]?.includes(item)} onChange={() => handleFilterToggle(sectionKey, item)} 
                className="w-3.5 h-3.5 rounded border-gray-300 text-gray-800" />
    ...
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={200000} step={5000}
          sx={{ color: "#1a1a1a", height: 4, '& .MuiSlider-thumb': { width: 18, height: 18, backgroundColor: '#1a1a1a' } }} />
      </div>
    </div>
  );

  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-gray-800"></div></div>;

  return (
    <div className="bg-gray-100 min-h-screen">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-gray-800 to-black text-white text-center py-12">
        <h1 className="text-4xl font-bold"> iPhone Collection</h1>
        <p className="mt-2 text-white/80">Experience the power of Apple iPhones</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">iPhone</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">iPhone</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No iPhone Products Found</h3>
