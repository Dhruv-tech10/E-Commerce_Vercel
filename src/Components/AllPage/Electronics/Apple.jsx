// src/component/AppleMacbook.jsx
...
const FILTER_CONFIG = {
  ...
  processor: ["Apple M1", "Apple M1 Pro", "Apple M1 Max", "Apple M1 Ultra", "Apple M2", "Apple M2 Pro", "Apple M2 Max", "Apple M3", "Apple M3 Pro", "Apple M3 Max"],
  display: ["13.3 inch", "14 inch", "15.3 inch", "16 inch"],
  color: ["Silver", "Space Gray", "Gold", "Midnight", "Starlight", "Rose Gold", "Sky Blue"],
  size: ["MacBook Air", "MacBook Pro 13", "MacBook Pro 14", "MacBook Pro 16"]
};
...
function Apple() {
  ...
  const [filters, setFilters] = useState({ ram: [], storage: [], processor: [], display: [], color: [], size: [] });
  const [openSections, setOpenSections] = useState({ 
    ram: true, storage: true, processor: true, display: false, color: false, size: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ ram: [], storage: [], processor: [], display: [], color: [], size: [] });
    setMaxPrice(400000);
    setSortBy("default");
  };
          ...
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
              <input type="checkbox" checked={filters[sectionKey]?.includes(item)} onChange={() => handleFilterToggle(sectionKey, item)} 
                className="w-3.5 h-3.5 rounded border-gray-300 text-gray-700" />
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Model" sectionKey="size" items={FILTER_CONFIG.size} />
      <div className="mb-4 pt-2">
        <h2 className="font-bold text-sm mb-2"> Max Price: ₹{maxPrice}</h2>
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={500000} step={10000}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-gray-700 to-gray-900 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Apple MacBook</h1>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Apple</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Apple</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No MacBook Products Found</h3>
