...
const FILTER_CONFIG = {
  series: ["Noise ColorFit Pro", "Noise ColorFit Ultra", "Noise ColorFit Vision", "Noise ColorFit Pulse", "Noise ColorFit Icon", "Noise ColorFit Grand", "Noise ColorFit Thrill", "Noise ColorFit Smart", "Noise Luna", "Noise Twist"],
  displaySize: ["1.3 inch", "1.4 inch", "1.69 inch", "1.78 inch", "1.85 inch", "1.96 inch", "2.01 inch"],
  color: ["Black", "Blue", "Green", "Red", "Silver", "Gold", "Pink", "Purple", "Teal", "Orange", "White"],
  connectivity: ["Bluetooth Calling", "Non-Calling"],
  batteryLife: ["5 Days", "7 Days", "10 Days", "12 Days", "15 Days", "20 Days", "30 Days"],
  strapMaterial: ["Silicone", "Metal", "Leather", "Nylon", "Rubber", "Fluoroelastomer"],
  waterResistance: ["IP67", "IP68", "5 ATM", "10 ATM"]
};
...
function Noice() {
  ...
  let filteredProducts = products.filter(p => 
    p.brand?.toLowerCase() === "noise" ||
    p.category?.toLowerCase() === "smartwatches" ||
    p.category?.toLowerCase() === "smart watches" ||
    p.name?.toLowerCase().includes("noise") ||
    p.name?.toLowerCase().includes("colorfit")
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Noise Smartwatches</h1>
       
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Noise Smartwatches</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Noise Smartwatches</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Noise Products Found</h3>
