...
const FILTER_CONFIG = {
  brand: ["Prestige", "Pigeon", "Philips", "Bajaj", "Usha", "Havells", "Butterfly", "Wonderchef", "Panasonic", "Kent", "Inalsa", "Morphy Richards", "Maharaja Whiteline", "Orient", "Crompton", "V-Guard", "Singer", "Bosch", "Glen", "Sunflame"],
  type: ["Induction Cooktop", "Electric Stove", "Electric Hob", "Ceramic Cooktop", "Glass Top Cooktop", "Portable Induction Cooker", "Built-in Hob", "2 Burner", "3 Burner", "4 Burner"],
  burnerCount: ["1 Burner", "2 Burners", "3 Burners", "4 Burners", "5 Burners", "6 Burners"],
  power: ["1200W", "1500W", "1800W", "2000W", "2100W", "2200W", "2400W", "2500W", "2600W", "2800W", "3000W", "3500W", "4000W", "5000W", "6000W", "7000W", "8000W"],
  color: ["Black", "White", "Silver", "Gray", "Red", "Blue", "Stainless Steel", "Glass Black", "Black Glass"],
  material: ["Glass", "Ceramic Glass", "Stainless Steel", "Toughened Glass", "Aluminum", "Cast Iron", "Steel Body"],
  controlType: ["Touch Control", "Knob Control", "Slider Control", "Digital Display", "Analog Control"],
  features: ["Timer Function", "Child Lock", "Overheat Protection", "Auto Shut Off", "Temperature Control", "Power Indicator", "LED Display", "Digital Screen", "Preset Menus", "Dual Circuit", "Pan Detection", "Voltage Protection", "Energy Efficient", "Non-Slip Feet", "Cooling Fan"]
};

function ElectricGas() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], burnerCount: [], power: [], color: [], material: [], controlType: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, burnerCount: false, power: true, color: false, material: false, controlType: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], burnerCount: [], power: [], color: [], material: [], controlType: [], features: [] });
    setMaxPrice(20000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "electric gas" ||
    p.category?.toLowerCase() === "induction cooktop" ||
    p.category?.toLowerCase() === "electric stove" ||
    p.subCategory?.toLowerCase() === "electric-gas" ||
    p.name?.toLowerCase().includes("induction") ||
    p.name?.toLowerCase().includes("electric stove") ||
    p.name?.toLowerCase().includes("cooktop") ||
    p.name?.toLowerCase().includes("electric gas")
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.burnerCount.length > 0 && !filters.burnerCount.includes(product.burnerCount)) return false;
    if (filters.power.length > 0 && !filters.power.includes(product.power)) return false;
    ...
    if (filters.controlType.length > 0 && !filters.controlType.includes(product.controlType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Burner Count" sectionKey="burnerCount" items={FILTER_CONFIG.burnerCount} />
      <AccordionSection title=" Power" sectionKey="power" items={FILTER_CONFIG.power} />
      ...
      <AccordionSection title=" Control Type" sectionKey="controlType" items={FILTER_CONFIG.controlType} />
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={50000} step={1000}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold">Electric Gas Stoves</h1>
   
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Electric Gas </span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Electric Gas</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Electric Gas Stoves Found</h3>
