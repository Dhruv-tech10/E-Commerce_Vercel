// src/component/MobileChargers.jsx
...
const FILTER_CONFIG = {
  brand: ["Samsung", "Apple", "OnePlus", "Xiaomi", "Realme", "boAt", "Portronics", "Ambrane", "Syska", "Zebronics", "Mi", "Motorola", "Google", "Nothing", "CMF", "Anker", "Belkin", "Spigen"],
  type: ["Wall Charger", "Fast Charger", "Travel Charger", "Car Charger", "Desktop Charger", "Multi-Port Charger", "GaN Charger", "Wireless Charger", "Magsafe Charger", "Power Delivery", "Quick Charge"],
  portType: ["USB-A", "USB-C", "Lightning", "Micro USB", "USB-C + USB-A", "Dual USB-C", "Triple USB-A", "Multiple Ports"],
  powerOutput: ["5W", "10W", "15W", "18W", "20W", "25W", "30W", "33W", "45W", "65W", "100W", "120W", "150W", "200W"],
  color: ["Black", "White", "Silver", "Gray", "Blue", "Red", "Green", "Purple"],
  cableType: ["Without Cable", "With USB-C Cable", "With Lightning Cable", "With Micro USB Cable", "Detachable Cable"],
  safetyFeatures: ["Overcharge Protection", "Short Circuit Protection", "Temperature Control", "Surge Protection", "Fire Resistant"]
};

function MobileChargers() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], portType: [], powerOutput: [], color: [], cableType: [], safetyFeatures: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, portType: false, powerOutput: true, color: false, cableType: false, safetyFeatures: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], portType: [], powerOutput: [], color: [], cableType: [], safetyFeatures: [] });
    setMaxPrice(5000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "chargers" ||
    p.category?.toLowerCase() === "mobile chargers" ||
    p.subCategory?.toLowerCase() === "mobile-chargers" ||
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.portType.length > 0 && !filters.portType.includes(product.portType)) return false;
    ...
    if (filters.cableType.length > 0 && !filters.cableType.includes(product.cableType)) return false;
    if (filters.safetyFeatures.length > 0 && !filters.safetyFeatures.includes(product.safetyFeatures)) return false;
    return true;
  });
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Port Type" sectionKey="portType" items={FILTER_CONFIG.portType} />
      ...
      <AccordionSection title=" Cable Type" sectionKey="cableType" items={FILTER_CONFIG.cableType} />
      <AccordionSection title=" Safety Features" sectionKey="safetyFeatures" items={FILTER_CONFIG.safetyFeatures} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold">Mobile Chargers</h1>
        
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Mobile Chargers</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Mobile Chargers</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Mobile Chargers Found</h3>
