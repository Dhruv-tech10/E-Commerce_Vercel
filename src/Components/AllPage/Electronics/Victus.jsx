...
const FILTER_CONFIG = {
  ...
  display: ["15.6 inch", "16.1 inch", "17.3 inch"],
  graphics: ["NVIDIA GeForce GTX 1650", "NVIDIA GeForce RTX 3050", "NVIDIA GeForce RTX 4050", "NVIDIA GeForce RTX 4060", "NVIDIA GeForce RTX 4070"],
  color: ["Black", "Silver", "Blue", "White", "Gray"]
};
...
function Victus() {
  ...
  let filteredProducts = products.filter(p => 
    ...
    p.name?.toLowerCase().includes("victus") ||
    p.name?.toLowerCase().includes("hp victus") ||
    (p.name?.toLowerCase().includes("gaming") && p.name?.toLowerCase().includes("hp"))
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-blue-700 to-blue-900 text-white text-center py-12">
        <h1 className="text-4xl font-bold">HP Victus Gaming Laptops</h1>
       
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Victus</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Victus</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Victus Laptops Found</h3>
