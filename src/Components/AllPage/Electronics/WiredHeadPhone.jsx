// src/component/WiredHeadphones.jsx
...
const FILTER_CONFIG = {
  brand: ["Sony", "JBL", "Sennheiser", "Audio-Technica", "Beyerdynamic", "Bose", "Skullcandy", "Razer", "Corsair", "HyperX", "Logitech", "Philips", "Panasonic", "boAt", "Realme", "OnePlus"],
  type: ["Over-Ear", "On-Ear", "In-Ear", "Earbuds", "Gaming Headphones", "Studio Headphones", "Noise Isolating"],
  connector: ["3.5mm Jack", "6.35mm Jack", "USB Type-A", "USB Type-C", "Lightning", "Micro USB"],
  color: ["Black", "White", "Gray", "Red", "Blue", "Green", "Brown", "Silver", "Gold", "Purple"],
  cableLength: ["1.2m", "1.5m", "2m", "2.5m", "3m", "5m", "10m"],
  impedance: ["16 ohms", "32 ohms", "64 ohms", "80 ohms", "250 ohms"],
  frequency: ["20Hz-20kHz", "20Hz-40kHz", "10Hz-25kHz", "5Hz-30kHz", "15Hz-28kHz"]
};
...
function WiredHeadphones() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], connector: [], color: [], cableLength: [], impedance: [], frequency: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, connector: false, color: false, cableLength: false, impedance: false, frequency: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], connector: [], color: [], cableLength: [], impedance: [], frequency: [] });
    setMaxPrice(50000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "headphones" ||
    p.category?.toLowerCase() === "wired headphones" ||
    p.category?.toLowerCase() === "wired-earphone" ||
    p.subCategory?.toLowerCase() === "headphones" ||
    p.name?.toLowerCase().includes("headphone") ||
    p.name?.toLowerCase().includes("wired")
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.connector.length > 0 && !filters.connector.includes(product.connector)) return false;
    ...
    if (filters.impedance.length > 0 && !filters.impedance.includes(product.impedance)) return false;
    if (filters.frequency.length > 0 && !filters.frequency.includes(product.frequency)) return false;
    return true;
  });
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Connector" sectionKey="connector" items={FILTER_CONFIG.connector} />
      ...
      <AccordionSection title=" Impedance" sectionKey="impedance" items={FILTER_CONFIG.impedance} />
      <AccordionSection title=" Frequency Range" sectionKey="frequency" items={FILTER_CONFIG.frequency} />
      <div className="mb-4 pt-2">
        <h2 className="font-bold text-sm mb-2"> Max Price: ₹{maxPrice}</h2>
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={80000} step={500}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold">Wired Headphones</h1>
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        ...
        <span className="text-lg !font-medium text-[#E4921A]">Wired Headphones</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Wired Headphones</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Wired Headphones Found</h3>
