...
const FILTER_CONFIG = {
  brand: ["Philips", "Bajaj", "Usha", "Havells", "Morphy Richards", "Panasonic", "Pigeon", "Black+Decker", "Tefal", "Rowenta", "Singer", "Electrolux", "Crompton", "Orient", "Inalsa", "Kenstar", "V-Guard", "Butterfly"],
  type: ["Steam Iron", "Dry Iron", "Garment Steamer", "Travel Iron", "Professional Iron", "Automatic Iron", "Cordless Iron", "Smart Iron"],
  soleplate: ["Non-Stick", "Ceramic", "Stainless Steel", "Titanium", "Ceramic Coated", "Aluminum", "Hard Anodized", "Durilium", "Ceramic Glide", "SteamGlide"],
  wattage: ["1000W", "1200W", "1500W", "1800W", "2000W", "2200W", "2400W", "2600W"],
  color: ["White", "Black", "Blue", "Red", "Purple", "Pink", "Green", "Silver", "Gray", "Teal"],
  steamOutput: ["Up to 20g/min", "Up to 30g/min", "Up to 40g/min", "Up to 50g/min", "Up to 60g/min", "Up to 70g/min", "Up to 80g/min", "Up to 100g/min"],
  waterTank: ["50ml", "100ml", "150ml", "200ml", "250ml", "300ml", "350ml", "400ml", "500ml", "Detachable Tank"],
  features: ["Anti-Drip", "Auto Shut Off", "Self-Clean", "Calc Clean", "Vertical Steam", "Spray Function", "Burst of Steam", "Temperature Control", "LED Indicator", "Cordless", "Swivel Cord", "Long Cord", "Cool Touch Handle", "Carrying Lock", "Transparent Tank"]
};
...
function Iron() {
  ...
  const [filters, setFilters] = useState({ brand: [], type: [], soleplate: [], wattage: [], color: [], steamOutput: [], waterTank: [], features: [] });
  const [openSections, setOpenSections] = useState({ 
    brand: true, type: true, soleplate: false, wattage: true, color: false, steamOutput: false, waterTank: false, features: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ brand: [], type: [], soleplate: [], wattage: [], color: [], steamOutput: [], waterTank: [], features: [] });
    setMaxPrice(10000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "iron" ||
    p.subCategory?.toLowerCase() === "iron" ||
    p.name?.toLowerCase().includes("iron") ||
    p.name?.toLowerCase().includes("steam iron") ||
    p.name?.toLowerCase().includes("garment steamer")
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.soleplate.length > 0 && !filters.soleplate.includes(product.soleplate)) return false;
    ...
    if (filters.steamOutput.length > 0 && !filters.steamOutput.includes(product.steamOutput)) return false;
    if (filters.waterTank.length > 0 && !filters.waterTank.includes(product.waterTank)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Soleplate" sectionKey="soleplate" items={FILTER_CONFIG.soleplate} />
      ...
      <AccordionSection title=" Steam Output" sectionKey="steamOutput" items={FILTER_CONFIG.steamOutput} />
      <AccordionSection title=" Water Tank" sectionKey="waterTank" items={FILTER_CONFIG.waterTank} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-red-600 to-orange-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Irons & Steam Irons</h1>
     
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Irons</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Irons</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Irons Found</h3>
