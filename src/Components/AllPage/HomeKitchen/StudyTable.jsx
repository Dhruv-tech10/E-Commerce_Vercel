...
const FILTER_CONFIG = {
  material: ["Wood", "Metal", "Glass", "MDF"],
  color: ["Brown", "White", "Black", "Gray"],
  sizes: ["Small", "Medium", "Large"],
  brands: ["Decorify", "IKEA", "Urban Ladder"]
};
...
function StudyTable() {
  ...
  const [filters, setFilters] = React.useState({ material: [], color: [], sizes: [], brands: [] });
  const [openSections, setOpenSections] = React.useState({ material: true, color: true, sizes: true, brands: true });
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold"> Study Table Collection</h1>
        <p className="mt-2 text-white/80">Study in style with our comfortable study tables</p>
      </div>
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Study Tables</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Study Tables</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">No Study Tables found.</div>
            ) : (
