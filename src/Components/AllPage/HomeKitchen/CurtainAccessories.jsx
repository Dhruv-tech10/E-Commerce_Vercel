// src/component/CurtainsAccessories.jsx
...
const FILTER_CONFIG = {
  material: ["Cotton", "Polyester", "Linen", "Silk", "Velvet", "Satin", "Nylon", "Sheer"],
  color: ["White", "Beige", "Gray", "Blue", "Green", "Red", "Black", "Gold", "Multicolor"],
  sizes: ["Small", "Medium", "Large", "Extra Large", "Window", "Floor Length", "Door"],
  styles: ["Rod Pocket", "Grommet", "Tab Top", "Back Tab", "Ripple Fold", "Pleated", "Panel"],
  brands: ["Home Centre", "Urban Ladder", "IKEA", "Pepperfry", "Amazon Basics", "FabIndia"]
};

function CurtainsAccessories() {
  ...
  const [filters, setFilters] = useState({ material: [], color: [], sizes: [], styles: [], brands: [] });
  const [openSections, setOpenSections] = useState({ material: true, color: true, sizes: true, styles: true, brands: true });
  ...
  const clearAllFilters = () => {
    setFilters({ material: [], color: [], sizes: [], styles: [], brands: [] });
    setMaxPrice(4000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "curtains" || 
    p.subCategory?.toLowerCase() === "curtains-accessories" ||
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-purple-600">
        <span>{title}</span>
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Style" sectionKey="styles" items={FILTER_CONFIG.styles} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold"> Curtains & Accessories</h1>
        <p className="mt-2 text-white/80">Beautiful curtains to enhance your home decor</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Curtains & Accessories</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Curtains & Accessories</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Curtains found.</div>
            ) : (
...
export default CurtainsAccessories;