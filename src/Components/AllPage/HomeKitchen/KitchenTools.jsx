// src/component/KitchenTools.jsx
...
const FILTER_CONFIG = {
  material: ["Stainless Steel", "Plastic", "Silicone", "Wood", "Nylon", "Aluminum"],
  color: ["Black", "Silver", "Red", "Blue", "Green", "White", "Multicolor"],
  sizes: ["Small", "Medium", "Large", "Set of 3", "Set of 5", "Set of 10"],
  brands: ["Prestige", "Pigeon", "Cello", "Milton", "IKEA", "Solimo", "Amazon Basics"]
};
...
function KitchenTools() {
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "kitchentools" || 
    p.subCategory?.toLowerCase() === "kitchen-tools" ||
    p.name?.toLowerCase().includes("spatula") ||
    p.name?.toLowerCase().includes("whisk") ||
    p.name?.toLowerCase().includes("peeler")
  );
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-teal-600">
        <span>{title}</span>
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-teal-600 to-cyan-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Kitchen Tools Collection</h1>
        <p className="mt-2 text-white/80">Essential tools for your daily cooking</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Kitchen Tools</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Kitchen Tools</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Kitchen Tools found.</div>
            ) : (
