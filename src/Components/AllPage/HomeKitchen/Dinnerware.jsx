...
const FILTER_CONFIG = {
  material: ["Bone China", "Ceramic", "Melamine", "Glass", "Stoneware", "Porcelain", "Stainless Steel"],
  color: ["White", "Black", "Blue", "Red", "Green", "Multicolor", "Gold", "Silver"],
  sizes: ["4 Piece", "6 Piece", "8 Piece", "12 Piece", "16 Piece", "20 Piece", "24 Piece"],
  brands: ["Borosil", "Cello", "Milton", "Pigeon", "IKEA", "Home Centre", "Corelle"]
};
...
function Dinnerware() {
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "dinnerware" || 
    p.subCategory?.toLowerCase() === "dinnerware" ||
    p.name?.toLowerCase().includes("plate") ||
    p.name?.toLowerCase().includes("bowl") ||
    p.name?.toLowerCase().includes("dinner set")
  );
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Pieces" sectionKey="sizes" items={FILTER_CONFIG.sizes} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold"> Dinnerware Collection</h1>
        <p className="mt-2 text-white/80">Elegant dinner sets for your daily meals</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Dinnerware</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Dinnerware</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Dinnerware found.</div>
            ) : (
