// src/component/ShowpiecesIdols.jsx
...
const FILTER_CONFIG = {
  material: ["Resin", "Marble", "Wood", "Metal", "Glass", "Ceramic", "Stone", "Brass", "Crystal"],
  color: ["Gold", "Brown", "White", "Black", "Multicolor", "Silver", "Bronze", "Beige"],
  sizes: ["Small", "Medium", "Large", "Extra Large", "Tabletop", "Floor Standing"],
  brands: ["Home Centre", "Urban Ladder", "Pepperfry", "IKEA", "ArtKarve", "TraditionArt"]
};

function ShowpiecesIdols() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category?.toLowerCase() === "showpieces" || 
           p.subCategory?.toLowerCase() === "showpieces-idols" ||
           (p.name?.toLowerCase().includes("showpiece")) ||
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Showpieces & Idols Collection</h1>
        <p className="mt-2 text-white/80">Elegant showpieces and idols for your home</p>
      </div>

      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Showpieces & Idols</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Showpieces & Idols</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Showpieces & Idols found.</div>
            ) : (
...
export default ShowpiecesIdols;