// src/component/DoormatsCarpets.jsx
...
const FILTER_CONFIG = {
  material: ["Cotton", "Polyester", "Jute", "Rubber", "Nylon", "Wool", "Bamboo", "Coir"],
  color: ["Brown", "Beige", "Gray", "Black", "Red", "Blue", "Green", "Multicolor"],
  shapes: ["Rectangular", "Round", "Square", "Oval", "Runner", "Door Mat"],
  sizes: ["Small", "Medium", "Large", "Extra Large", "Runner"],
  brands: ["Home Centre", "Urban Ladder", "IKEA", "Pepperfry", "Amazon Basics"]
};

function CarpetsDoormats() {
  ...
  const [filters, setFilters] = useState({ material: [], color: [], shapes: [], sizes: [], brands: [] });
  const [openSections, setOpenSections] = useState({ material: true, color: true, shapes: true, sizes: true, brands: true });
  ...
  const clearAllFilters = () => {
    setFilters({ material: [], color: [], shapes: [], sizes: [], brands: [] });
    setMaxPrice(2000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "doormats" || 
    p.subCategory?.toLowerCase() === "doormats-carpets" ||
    p.name?.toLowerCase().includes("doormat") ||
    p.name?.toLowerCase().includes("carpet") ||
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Shape" sectionKey="shapes" items={FILTER_CONFIG.shapes} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-green-600 to-teal-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Doormats & Carpets</h1>
        <p className="mt-2 text-white/80">Welcome guests with our beautiful doormats and carpets</p>
      </div>
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Doormats & Carpets</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Doormats & Carpets</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Doormats & Carpets found.</div>
            ) : (
...
export default CarpetsDoormats;