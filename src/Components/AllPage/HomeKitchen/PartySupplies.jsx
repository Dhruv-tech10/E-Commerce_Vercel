// src/component/PartySupplies.jsx
...
const FILTER_CONFIG = {
  material: ["Paper", "Plastic", "Latex", "Foil", "Fabric", "Cardboard", "Glass", "Metal"],
  color: ["Multicolor", "Red", "Gold", "Silver", "Blue", "Pink", "Yellow", "Green", "Purple"],
  sizes: ["Small", "Medium", "Large", "One Size", "Pack of 10", "Pack of 20", "Pack of 50"],
  brands: ["PartyMania", "DecoJoy", "Celebrate", "Amazon Basics", "Home Centre"]
};
...
function PartySupplies() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category?.toLowerCase() === "partysupplies" || 
           p.subCategory?.toLowerCase() === "party-supplies" ||
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Quantity" sectionKey="sizes" items={FILTER_CONFIG.sizes} />
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={8000} step={100}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-pink-600 to-rose-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">🎉 Party Supplies Collection</h1>
        <p className="mt-2 text-white/80">Everything you need for your next party</p>
      </div>
 <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Party Supplies</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Party Supplies</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Party Supplies found.</div>
            ) : (
