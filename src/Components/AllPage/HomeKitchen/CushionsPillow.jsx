// src/component/PillowCushionCovers.jsx
...
const FILTER_CONFIG = {
  material: ["Cotton", "Linen", "Velvet", "Silk", "Polyester", "Jute", "Wool", "Faux Fur"],
  color: ["White", "Beige", "Gray", "Blue", "Green", "Pink", "Red", "Yellow", "Multicolor"],
  sizes: ["12x12", "14x14", "16x16", "18x18", "20x20", "24x24", "Lumbar"],
  patterns: ["Solid", "Printed", "Striped", "Floral", "Geometric", "Embroidered", "Bohemian"],
  filling: ["Cotton", "Polyfill", "Memory Foam", "Down", "Microfiber", "Bamboo"],
  brands: ["Home Centre", "Urban Ladder", "IKEA", "Pepperfry", "Wakefit", "Amazon Basics"]
};

function CushionsPillow() {
  ...
  const [filters, setFilters] = useState({ material: [], color: [], sizes: [], patterns: [], filling: [], brands: [] });
  const [openSections, setOpenSections] = useState({ material: true, color: true, sizes: true, patterns: true, filling: true, brands: true });
  ...
  const clearAllFilters = () => {
    setFilters({ material: [], color: [], sizes: [], patterns: [], filling: [], brands: [] });
    setMaxPrice(2000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "pillows" || 
    p.subCategory?.toLowerCase() === "pillow-cushion-covers" ||
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-pink-600">
        <span>{title}</span>
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Pattern" sectionKey="patterns" items={FILTER_CONFIG.patterns} />
      <AccordionSection title=" Filling" sectionKey="filling" items={FILTER_CONFIG.filling} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-pink-600 to-rose-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Pillow, Cushion & Covers</h1>
        <p className="mt-2 text-white/80">Comfortable pillows and stylish cushion covers</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Pillow, Cushion & Covers</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Pillow, Cushion & Covers</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Pillows & Cushions found.</div>
            ) : (
