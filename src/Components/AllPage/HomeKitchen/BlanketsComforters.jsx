// src/component/BlanketsComforters.jsx
...
const FILTER_CONFIG = {
  material: ["Cotton", "Microfiber", "Wool", "Fleece", "Polyester", "Silk", "Bamboo", "Down", "Down Alternative"],
  color: ["White", "Gray", "Blue", "Beige", "Red", "Green", "Pink", "Navy", "Brown"],
  sizes: ["Twin", "Full", "Queen", "King", "California King", "Throw", "Blanket"],
  weight: ["Lightweight", "Medium Weight", "Heavy Weight", "All Season", "Winter", "Summer"],
  brands: ["Home Centre", "Urban Ladder", "Wakefit", "IKEA", "Pepperfry", "Amazon Basics", "Solimo"]
};

function BlanketsComforters() {
  ...
  const [filters, setFilters] = useState({ material: [], color: [], sizes: [], weight: [], brands: [] });
  const [openSections, setOpenSections] = useState({ material: true, color: true, sizes: true, weight: true, brands: true });
  ...
  const clearAllFilters = () => {
    setFilters({ material: [], color: [], sizes: [], weight: [], brands: [] });
    setMaxPrice(5000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "blankets" || 
    p.subCategory?.toLowerCase() === "blankets-comforters" ||
  ...
  filteredProducts = filteredProducts.filter(p => {
    ...
    if (filters.weight.length && !filters.weight.includes(p.weight)) return false;
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-red-600">
        <span>{title}</span>
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={15000} step={300}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-red-600 to-orange-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Blankets & Comforters</h1>
        <p className="mt-2 text-white/80">Stay warm and cozy with our premium blankets</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Blankets & Comforters</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Blankets & Comforters</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Blankets & Comforters found.</div>
            ) : (
...
export default BlanketsComforters;