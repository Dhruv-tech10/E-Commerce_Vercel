...
const FILTER_CONFIG = {
  material: ["Cotton", "Polyester", "Canvas", "Nylon", "Jute", "Rope", "Wood", "Metal", "Bamboo", "Fabric"],
  color: ["Multicolor", "White", "Brown", "Black", "Blue", "Green", "Red", "Yellow", "Orange", "Pink"],
  sizes: ["Single", "Double", "Large Double", "Family Size", "Kids Size", "Compact", "Extra Large"],
  brands: ["Decorify", "Outdoor Life", "Relax Home", "Amazon Basics", "Pepperfry", "Hammock World", "Swing Zone"]
};

function HammocSwing() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category?.toLowerCase() === "hammocswing" || 
           p.subCategory?.toLowerCase() === "hammock-swing" ||
           p.type?.toLowerCase() === "hammock" ||
           p.type?.toLowerCase() === "swing" ||
           (p.name?.toLowerCase().includes("hammock")) ||
           (p.name?.toLowerCase().includes("swing")) ||
           (p.name?.toLowerCase().includes("hanging chair"))
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-teal-600 to-emerald-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Hammock & Swing Collection</h1>
        <p className="mt-2 text-white/80">Relax and unwind with our comfortable hammocks and swings</p>
      </div>

      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Hammocks & Swings</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Hammocks & Swings</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Hammocks or Swings found.</div>
            ) : (
