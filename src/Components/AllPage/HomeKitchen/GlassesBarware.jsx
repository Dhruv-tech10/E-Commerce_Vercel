// src/component/GlassesBarware.jsx
...
const FILTER_CONFIG = {
  material: ["Glass", "Crystal", "Stainless Steel", "Plastic", "Copper", "Ceramic"],
  color: ["Clear", "Blue", "Green", "Red", "Black", "Gold", "Silver", "Multicolor"],
  sizes: ["Small", "Medium", "Large", "Set of 4", "Set of 6", "Set of 12"],
  brands: ["Borosil", "Cello", "Milton", "Pigeon", "IKEA", "Home Centre"]
};

function GlassesBarware() {
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "glasses" || 
    p.subCategory?.toLowerCase() === "glasses-barware" ||
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-cyan-600">
        <span>{title}</span>
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={6000} step={100}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Glasses & Barware</h1>
        <p className="mt-2 text-white/80">Premium glassware for every occasion</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Glasses & Barware</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Glasses & Barware</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Glasses & Barware found.</div>
            ) : (
