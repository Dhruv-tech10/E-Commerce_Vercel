// src/component/BathroomAccessories.jsx
...
const FILTER_CONFIG = {
  material: ["Plastic", "Stainless Steel", "Glass", "Ceramic", "Bamboo", "Wood", "Acrylic", "Resin"],
  color: ["White", "Black", "Gray", "Silver", "Blue", "Green", "Beige", "Multicolor"],
  sizes: ["Small", "Medium", "Large", "Set of 2", "Set of 3", "Set of 4", "Set of 5"],
  brands: ["Home Centre", "Urban Ladder", "IKEA", "Amazon Basics", "Cello", "Milton", "Prestige"]
};

function BathroomAccessories() {
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "bathroom" || 
    p.subCategory?.toLowerCase() === "bathroom-accessories" ||
    p.name?.toLowerCase().includes("bathroom") ||
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-blue-600">
        <span>{title}</span>
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold"> Bathroom Accessories</h1>
        <p className="mt-2 text-white/80">Complete your bathroom with our premium accessories</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Bathroom Accessories</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Bathroom Accessories</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Bathroom Accessories found.</div>
            ) : (
