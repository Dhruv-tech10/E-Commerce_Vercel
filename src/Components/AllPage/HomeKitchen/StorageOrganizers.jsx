// src/component/StorageOrganizers.jsx
...
const FILTER_CONFIG = {
  material: ["Plastic", "Glass", "Stainless Steel", "Bamboo", "Wood", "Silicone", "Ceramic"],
  color: ["Clear", "White", "Blue", "Green", "Red", "Black", "Multicolor"],
  sizes: ["Small", "Medium", "Large", "Extra Large", "Set of 3", "Set of 5", "Set of 10"],
  brands: ["Prestige", "Pigeon", "Borosil", "Milton", "Cello", "IKEA", "Home Centre"]
};

function StorageOrganizers() {
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "storage" || 
    p.subCategory?.toLowerCase() === "storage-organizers" ||
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-orange-600">
        <span>{title}</span>
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold"> Storage & Organizers</h1>
        <p className="mt-2 text-white/80">Keep your home organized with our storage solutions</p>
      </div>
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Storage & Organizers</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Storage & Organizers</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Storage & Organizers found.</div>
            ) : (
...
export default StorageOrganizers;