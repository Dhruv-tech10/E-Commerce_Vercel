...
const FILTER_CONFIG = {
  material: ["Wood", "Metal", "Plastic", "Acrylic", "Glass", "Ceramic", "Leather", "Bamboo", "Iron", "Brass"],
  color: ["Black", "Brown", "White", "Gold", "Silver", "Multicolor", "Blue", "Green", "Red"],
  sizes: ["Small", "Medium", "Large", "Wall Mounted", "Tabletop"],
  brands: ["Home Centre", "Urban Ladder", "Pepperfry", "IKEA", "Amazon Basics"]
};
...
function KeyHolders() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category?.toLowerCase() === "keyholders" || 
           p.subCategory?.toLowerCase() === "key-holders" ||
           (p.name?.toLowerCase().includes("key holder"))
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold">Key Holders Collection</h1>
        <p className="mt-2 text-white/80">Stylish key holders to organize your keys</p>
      </div>

      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Key Holders</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Key Holders</h2>
      ...
      <div className="max-w-7xl mx-auto p-4">
        <div className="flex gap-6">
          <div className="w-72 hidden md:block sticky top-20 h-[calc(100vh-5rem)] overflow-y-auto"><FilterSidebar /></div>
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">No Key Holders found.</div>
            ) : (
