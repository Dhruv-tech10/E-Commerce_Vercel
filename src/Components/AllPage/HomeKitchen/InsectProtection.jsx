// src/component/InsectProtection.jsx
...
const FILTER_CONFIG = {
  material: ["Plastic", "Metal", "Mesh", "Fabric", "Rubber", "Glass", "Paper"],
  color: ["White", "Blue", "Green", "Black", "Yellow", "Gray", "Multicolor"],
  sizes: ["Small", "Medium", "Large", "Pack of 2", "Pack of 3", "Pack of 4", "Pack of 5"],
  brands: ["Home Centre", "Amazon Basics", "Solimo", "Godrej", "Hit", "Mortein", "Goodknight"]
};

function InsectProtection() {
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "insect" || 
    p.subCategory?.toLowerCase() === "insect-protection" ||
    p.name?.toLowerCase().includes("mosquito") ||
    p.name?.toLowerCase().includes("insect") ||
    p.name?.toLowerCase().includes("repellent") ||
    p.name?.toLowerCase().includes("net")
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-red-600 to-orange-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Insect Protection</h1>
        <p className="mt-2 text-white/80">Protect your home from insects and mosquitoes</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Insect Protection</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Insect Protection</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Insect Protection products found.</div>
            ) : (
