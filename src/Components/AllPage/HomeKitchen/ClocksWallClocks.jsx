// src/component/ClocksWallDecor.jsx
...
const FILTER_CONFIG = {
  material: ["Wood", "Metal", "Plastic", "Glass", "Acrylic", "Ceramic", "Brass", "Iron"],
  color: ["Black", "White", "Brown", "Gold", "Silver", "Multicolor", "Blue", "Green", "Red"],
  sizes: ["Small", "Medium", "Large", "Extra Large", "Wall Mounted"],
  brands: ["Titan", "Fastrack", "Home Centre", "Urban Ladder", "Pepperfry", "IKEA"]
};

function ClocksWallDecor() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category?.toLowerCase() === "clocks" || 
           p.subCategory?.toLowerCase() === "clocks-wall-decor" ||
           (p.name?.toLowerCase().includes("clock")) ||
           (p.name?.toLowerCase().includes("wall decor"))
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-teal-600 to-emerald-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold"> Clocks & Wall Decor Collection</h1>
        <p className="mt-2 text-white/80">Beautiful clocks and wall decor items for your home</p>
      </div>
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Clocks & Wall Decor</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Clocks & Wall Decor</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Clocks & Wall Decor found.</div>
            ) : (
...
export default ClocksWallDecor;