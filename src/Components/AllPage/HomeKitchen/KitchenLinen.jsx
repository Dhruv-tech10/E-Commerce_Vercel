// src/component/KitchenLinen.jsx
...
const FILTER_CONFIG = {
  material: ["Cotton", "Microfiber", "Polyester", "Linen", "Jute", "Bamboo"],
  color: ["White", "Red", "Blue", "Green", "Yellow", "Pink", "Gray", "Multicolor"],
  sizes: ["Small", "Medium", "Large", "Set of 2", "Set of 4", "Set of 6"],
  brands: ["Home Centre", "IKEA", "Urban Ladder", "Amazon Basics", "Solimo"]
};
...
function KitchenLinen() {
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "kitchenlinen" || 
    p.subCategory?.toLowerCase() === "kitchen-linen" ||
    p.name?.toLowerCase().includes("apron") ||
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={4000} step={100}
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-pink-600 to-rose-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Kitchen Linen Collection</h1>
        <p className="mt-2 text-white/80">Soft and durable kitchen linens</p>
      </div>
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Kitchen Linen</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Kitchen Linen</h2>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center"><div className="text-5xl mb-4"></div>No Kitchen Linen found.</div>
            ) : (
