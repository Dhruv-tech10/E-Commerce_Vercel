// src/Components/AllPage/Icons/IconWomen.jsx
...
const FILTER_CONFIG = {
  // Categories
  categories: [
    "T-Shirt", "Tunic & Top", "Shirt", "Dresses", "Gowns", 
    "Dupatta", "Saree", "Jumpsuit", "Kurtie", "Blouses",
    "Jeans & Jeggie", "Legging", "Shorts & Skirt", "Palazzo", "Pants & Trouser",
    "Jackets", "Sweaters", "Coats", "Blazers",
    "Women Bra", "Women Panties", "Sports Bra",
    "Women NightDress", "Women NightSuit",
    "Anklets", "Bracelets", "Jwellery", "Earrings", "Watch", "Necklace",
    "Facewash", "Perfumes", "SunGlasses"
  ],
  
  // Fabric
  fabric: [
    "Cotton", "Silk", "Chiffon", "Georgette", "Satin",
    "Linen", "Velvet", "Crepe", "Jacquard", "Net",
    "Polyester", "Viscose", "Rayon", "Denim", "Wool",
    "Cashmere", "Pashmina", "Organza", "Taffeta", "Lace"
  ],
  
  // Size
  sizes: ["S", "M", "L", "XL", "XXL", "Free Size", "6", "7", "8", "9", "10"],
  ...
  // Brands
  brands: [
    "Biba", "Zara", "H&M", "Mango", "W", 
    "Allen Solly", "Levi's", "Nike", "Adidas", "Puma",
    "FabIndia", "Global Desi", "Anouk", "Aurelia", "Vero Moda",
    "Only", "Jack & Jones", "Tommy Hilfiger", "Calvin Klein"
  ]
};
...
function IconWomen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // ✅ Filter ALL Women's products from all subcategories
        const womenProducts = data.filter(p => {
          const category = p.category?.toLowerCase() || '';
          const name = p.name?.toLowerCase() || '';
          
          // All Women's categories from WomenRouter
          const womenCategories = [
            't-shirt', 'tunic', 'top', 'shirt', 'dresses', 'gowns', 
            'dupatta', 'saree', 'jumpsuit', 'kurtie', 'blouses',
            'jeans', 'jeggie', 'legging', 'shorts', 'skirt', 'palazzo', 'pants', 'trouser',
            'jackets', 'sweaters', 'coats', 'blazers',
            'bra', 'panties', 'sports bra',
            'nightdress', 'nightsuit',
            'anklets', 'bracelets', 'jwellery', 'earrings', 'watch', 'necklace',
            'facewash', 'perfumes', 'sunglasses', 'women'
          ];
          
          return womenCategories.some(womenCat => 
            category.includes(womenCat) || name.includes(womenCat)
          );
        });
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      {/* HEADER BANNER - Women's Theme */}
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Women's Collection</h1>
        <p className="mt-2 text-white/80 text-sm">Discover the Latest Trends in Women's Fashion</p>
      </div>

      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Women's Collection</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Women's Collection</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <p className="text-gray-500 text-sm">No Women's products found matching your filters.</p>
                <button 
