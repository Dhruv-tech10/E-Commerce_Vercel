// src/Components/AllPage/Icons/IconBags.jsx
...
const FILTER_CONFIG = {
  // Categories
  categories: [
    "Backpack", "Handbag", "Travel Bag", "Laptop Bag", 
    "Sling Bag", "Tote", "Wallet", "Duffle Bag",
    "Messenger Bag", "Crossbody Bag", "Clutch", "Shoulder Bag",
    "Trolley Bag", "Rucksack", "School Bag", "Gym Bag",
    "Briefcase", "Luggage", "Suitcase"
  ],
  ...
  // Color
  colors: [
    "Black", "Brown", "Navy", "Tan", "Grey", 
    "Red", "Blue", "Green", "Pink", "White",
    "Gold", "Silver", "Multi-Color", "Beige", "Camel"
  ],
  
  // Material
  materials: [
    "Leather", "PU Leather", "Canvas", "Nylon", 
    "Polyester", "Jute", "Cotton", "Suede",
    "Faux Leather", "Vegan Leather", "Denim", "Woven",
    "Mesh", "Oxford Fabric"
  ],
  
  // Brands
  brands: [
    "Skybags", "American Tourister", "Safari", "Wildcraft", 
    "Nike", "Adidas", "Puma", "Reebok",
    "Allen Solly", "Louis Philippe", "Van Heusen",
    "Fastrack", "Fossil", "Tommy Hilfiger", "Calvin Klein",
    "Heritage", "Caprese", "Lavie", "Baggit", "Hidesign",
    "Samsonite", "Delsey", "VIP"
  ]
};
...
function IconBags() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // ✅ Filter ALL Bags products
        const bagsProducts = data.filter(p => {
          const category = p.category?.toLowerCase() || '';
          const name = p.name?.toLowerCase() || '';
          
          const bagsCategories = [
            'backpack', 'handbag', 'travel bag', 'laptop bag', 
            'sling bag', 'tote', 'wallet', 'duffle',
            'messenger', 'crossbody', 'clutch', 'shoulder',
            'trolley', 'rucksack', 'school bag', 'gym bag',
            'briefcase', 'luggage', 'suitcase', 'bag', 'bags'
          ];
          
          return bagsCategories.some(bagCat => 
            category.includes(bagCat) || name.includes(bagCat)
          );
        });
        
        console.log("Bags Products Found:", bagsProducts.length);
  ...
  // ✅ FILTER PRODUCTS - LEFT FILTER KE HISAB SE RIGHT MEIN PRODUCTS
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button 
        ...
        className="flex justify-between items-center w-full font-bold text-gray-800 text-sm mb-2 hover:text-blue-600 transition"
      >
        <span>{title}</span>
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Bags Collection</h1>
        <p className="mt-2 text-white/80 text-sm">Carry Your Style with Our Premium Bags</p>
      </div>

      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 pl-7">
        <Link to="/" className="text-lg !no-underline font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg font-medium">/</span>
        <span className="text-lg font-medium text-[#E4921A]">Bags Collection</span>
      </nav>
      <h2 className="text-2xl font-bold mt-3 mb-3 pl-7">Bags Collection</h2>
          ...
          <div className="flex-1">
            <div className="bg-white rounded-xl shadow-sm p-3 mb-4 flex justify-between items-center flex-wrap gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <select 
                  ...
                  className="border rounded-lg px-3 py-1.5 text-xs font-medium focus:outline-none focus:border-blue-500"
                >
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <p className="text-gray-500 text-sm">No Bags products found matching your filters.</p>
                <button 
