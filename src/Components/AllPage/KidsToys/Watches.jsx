...
const FILTER_CONFIG = {
  // Watch Type
  watchType: [
    "Analog Watch", "Digital Watch", "Smart Watch", "Chronograph", 
    "Sports Watch", "Cartoon Watch", "LED Watch", "Touch Screen Watch",
    "Fitness Band", "Activity Tracker", "Basic Watch"
  ],
  
  // Size (Kids)
  size: ["S (Small)", "M (Medium)", "L (Large)", "Adjustable", "One Size", "Kids Fit"],
  
  // Color
  color: ["Black", "White", "Blue", "Red", "Pink", "Green", "Yellow", "Orange", "Purple", "Multicolor", "Silver", "Gold"],
  
  // Strap Material
  strapMaterial: ["Silicone", "Leather", "Metal", "Nylon", "Plastic", "Rubber", "Fabric"],
  
  // Dial Shape
  dialShape: ["Round", "Square", "Rectangle", "Hexagon", "Cartoon Shape", "Oval"],
  
  // Display Type
  displayType: ["LCD", "LED", "Analog Dial", "Digital Screen", "Touch Screen"],
  
  // Features
  features: ["Water Resistant", "Shock Proof", "Backlight", "Stopwatch", "Alarm", "Date Display", "Night Mode", "GPS", "Heart Rate Monitor", "Activity Tracking"],
  
  // Gender
  gender: ["Boys", "Girls", "Unisex"],
  
  // Brands
  brand: ["Fastrack", "Titan", "Casio", "Timex", "Sonata", "Fossil", "Apple", "Samsung", "Noise", "boAt", "Fire-Boltt", "Kurio", "VTech", "Disney", "Ben 10", "Spider-Man", "Frozen", "Cars"]
};
...
function KidsWatches() {
  ...
  const [filters, setFilters] = useState({ 
    watchType: [], size: [], color: [], strapMaterial: [], 
    dialShape: [], displayType: [], features: [], gender: [], brand: [] 
  });
  
  const [openSections, setOpenSections] = useState({
    watchType: true, size: true, color: true, strapMaterial: false,
    dialShape: false, displayType: false, features: false, gender: false, brand: false
  });
  ...
  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "kids watches" ||
    p.category?.toLowerCase() === "kids watch" ||
    p.subCategory?.toLowerCase() === "kids-watches" ||
    p.type?.toLowerCase() === "watch" ||
    p.name?.toLowerCase().includes("kids watch") ||
    (p.name?.toLowerCase().includes("watch") && p.name?.toLowerCase().includes("kids"))
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.displayType.length > 0 && !filters.displayType.includes(product.displayType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm sticky top-20">
      ...
      <AccordionSection title=" Display Type" sectionKey="displayType" items={FILTER_CONFIG.displayType} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-4xl font-bold"> Kids Watches Collection</h1>
      </div>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Kids Watches Found</h3>
