...
// 👦 BOYS FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  boysClothing: ["T-Shirts", "Shirts", "Jeans", "Shorts", "Track Pants", "Jackets", "Sweaters"],
  boysFootwear: ["Sports Shoes", "Casual Shoes", "Sandals", "Slippers"],
  boysAccessories: ["Watches", "Bags", "Belts", "Caps", "Sunglasses"],
  boysToys: ["Puzzle", "Toys & Games", "Remote Control", "Action Figures", "Cars", "Bikes", "Cricket"],
  sizes: ["2-3Y", "3-4Y", "4-5Y", "5-6Y", "6-7Y", "7-8Y", "8-10Y", "10-12Y"],
  brands: ["Nike", "Adidas", "Puma", "Levi's", "Gini & Jony", "Lilliput", "Babyhug", "Mee Mee", "FunSkool", "Fisher Price"]
};

// Sidebar configuration
const SIDEBAR_SECTIONS = [
  { title: "👕 Boys Clothing", key: "boysClothing" },
  { title: "👟 Boys Footwear", key: "boysFootwear" },
  { title: "🎒 Boys Accessories", key: "boysAccessories" },
  { title: "🧸 Boys Toys", key: "boysToys" },
...
function Boys() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const boysProducts = loadedProducts.filter(p =>
          p.category === "Boys" ||
          p.gender === "Boys" ||
          p.gender === "Male" ||
          p.for === "Boys"
        );
        setProducts(boysProducts.length > 0 ? boysProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(kidsProducts.filter(p => p.category === "Boys"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      const activeCategoryFilters = [
        ...filters.boysClothing,
        ...filters.boysFootwear,
        ...filters.boysAccessories,
        ...filters.boysToys
      ];
          ...
          <div className="flex gap-1.5 flex-wrap pt-1">
            {FILTER_CONFIG.sizes.map((size, index) => (
              <button
                ...
                className={`border px-2 py-1 rounded-md text-[11px] font-semibold transition ${filters.sizes.includes(size)
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm font-medium">Loading Boys Collection...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      {/* Header Hero Banner */}
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">👦 Boys Collection</h1>
        <p className="mt-2 text-blue-100 text-lg">Stylish & comfortable for every boy</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Boys</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Boys</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} boys items</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                ...
                No boys products found matching your criteria.
