...
// 👧 GIRLS FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  girlsClothing: ["Dresses", "Skirts", "Tops", "Jeans", "Shorts", "Leggings", "Jackets", "Sweaters"],
  girlsFootwear: ["Sports Shoes", "Casual Shoes", "Sandals", "Slippers", "Party Shoes"],
  girlsAccessories: ["Hair Bands", "Bags", "Belts", "Caps", "Sunglasses", "Jewelry"],
  girlsToys: ["Dolls", "Puzzle", "Toys & Games", "Teddy Bears", "Remote Control", "Art & Craft"],
  sizes: ["2-3Y", "3-4Y", "4-5Y", "5-6Y", "6-7Y", "7-8Y", "8-10Y", "10-12Y"],
  brands: ["Nike", "Adidas", "Puma", "Levi's", "Gini & Jony", "Lilliput", "Babyhug", "Mee Mee", "Barbie", "Disney"]
};

// Sidebar configuration
const SIDEBAR_SECTIONS = [
  { title: "👗 Girls Clothing", key: "girlsClothing" },
  { title: "👟 Girls Footwear", key: "girlsFootwear" },
  { title: "🎀 Girls Accessories", key: "girlsAccessories" },
  { title: "🧸 Girls Toys", key: "girlsToys" },
...
function Girls() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const girlsProducts = loadedProducts.filter(p => 
          p.category === "Girls" || 
          p.gender === "Girls" || 
          p.gender === "Female" ||
          p.for === "Girls"
        );
        setProducts(girlsProducts.length > 0 ? girlsProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(kidsProducts.filter(p => p.category === "Girls"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      const activeCategoryFilters = [
        ...filters.girlsClothing, 
        ...filters.girlsFootwear,
        ...filters.girlsAccessories, 
        ...filters.girlsToys
      ];
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm font-medium">Loading Girls Collection...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      {/* Header Hero Banner */}
      <div className="bg-gradient-to-r from-pink-500 to-rose-500 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">👧 Girls Collection</h1>
        <p className="mt-2 text-pink-100 text-lg">Beautiful & stylish for every girl</p>
      </div>
  <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Girls</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Girls</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} girls items</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                ...
                No girls products found matching your criteria.
