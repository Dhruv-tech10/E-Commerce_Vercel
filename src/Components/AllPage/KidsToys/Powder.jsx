...
// 🧴 POWDER FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  powderTypes: ["Baby Powder", "Talcum Powder", "Medicated Powder", "Natural Powder", "Fragrance Free Powder"],
  brands: ["Babyhug", "Mee Mee", "Lilliput", "Johnson's", "Pampers", "Huggies"]
};

// Sidebar configuration
const SIDEBAR_SECTIONS = [
  { title: "🧴 Powder Types", key: "powderTypes" },
...
function Powder() {
  ...
  const [filters, setFilters] = useState({ powderTypes: [], brands: [] });
  const [openSections, setOpenSections] = useState({ powderTypes: true, brands: true });
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const powderProducts = loadedProducts.filter(p => 
          p.category === "Powder" || 
          p.subCategory === "Powder" ||
          p.for === "Powder"
        );
        setProducts(powderProducts.length > 0 ? powderProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(kidsProducts.filter(p => p.category === "Powder"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      if (filters.powderTypes.length > 0 && !filters.powderTypes.includes(product.subCategory)) return false;
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600 mx-auto mb-3"></div><p className="text-gray-500 text-sm font-medium">Loading Powder Collection...</p></div></div>;

  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="bg-gradient-to-r from-green-500 to-emerald-500 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">🧴 Powder</h1>
        <p className="mt-2 text-green-100 text-lg">Soft and gentle powder for kids</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Powder</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Powder</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} powder items</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                ...
                No powder products found matching your criteria.
