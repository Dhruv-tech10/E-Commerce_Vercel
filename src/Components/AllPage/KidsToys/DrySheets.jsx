...
// 🧻 DRY SHEETS FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  drySheetTypes: ["Baby Dry Sheets", "Waterproof Dry Sheets", "Reusable Dry Sheets", "Disposable Dry Sheets", "Cotton Dry Sheets"],
  brands: ["Babyhug", "Mee Mee", "Lilliput", "Pampers", "Huggies", "Johnson's"]
};

// Sidebar configuration
const SIDEBAR_SECTIONS = [
  { title: "🧻 Dry Sheet Types", key: "drySheetTypes" },
...
function DrySheets() {
  ...
  const [filters, setFilters] = useState({ drySheetTypes: [], brands: [] });
  const [openSections, setOpenSections] = useState({ drySheetTypes: true, brands: true });
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const drySheetProducts = loadedProducts.filter(p => 
          p.category === "Dry Sheets" || 
          p.subCategory === "Dry Sheets" ||
          p.for === "Dry Sheets"
        );
        setProducts(drySheetProducts.length > 0 ? drySheetProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(kidsProducts.filter(p => p.category === "Dry Sheets"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const clearAllFilters = () => {
    setFilters({ drySheetTypes: [], brands: [] });
    setMaxPrice(5000);
  };
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      if (filters.drySheetTypes.length > 0 && !filters.drySheetTypes.includes(product.subCategory)) return false;
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600 mx-auto mb-3"></div><p className="text-gray-500 text-sm font-medium">Loading Dry Sheets Collection...</p></div></div>;

  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="bg-gradient-to-r from-teal-400 to-cyan-400 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">🧻 Dry Sheets</h1>
        <p className="mt-2 text-teal-100 text-lg">Comfortable dry sheets for babies</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Dry Sheets</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Dry Sheets</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} dry sheet items</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                ...
                No dry sheet products found matching your criteria.
