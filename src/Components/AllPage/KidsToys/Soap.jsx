...
// 🧼 SOAP FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  soapTypes: ["Baby Soap", "Herbal Soap", "Medicated Soap", "Natural Soap", "Moisturizing Soap"],
...
function Soap() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Filter only Soap products
        const soapProducts = loadedProducts.filter(p => 
          p.category === "Soap" || 
          p.subCategory === "Soap" ||
          p.for === "Soap"
        );
        setProducts(soapProducts.length > 0 ? soapProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(kidsProducts.filter(p => p.category === "Soap"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      const activeCategoryFilters = [...filters.soapTypes];
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm font-medium">Loading Soap Collection...</p>
      </div>
    </div>
  );
  ...
  <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Soap</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Soap</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} soap items</span>
