// src/component/RemoteControlHelicopters.jsx
...
// 🚁 REMOTE CONTROL HELICOPTERS FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  rcHelicopterTypes: ["Mini RC Helicopters", "Indoor RC Helicopters", "Outdoor RC Helicopters", "Stunt RC Helicopters", "Lighted RC Helicopters"],
  brands: ["FunSkool", "Fisher Price", "Lego", "Hot Wheels", "Barbie", "Disney"]
};

// Sidebar configuration
const SIDEBAR_SECTIONS = [
  { title: "🚁 RC Helicopter Types", key: "rcHelicopterTypes" },
  { title: "🏷️ Brands", key: "brands" }
];

function RemoteControlhelecopers() {
  ...
  const [filters, setFilters] = useState({ rcHelicopterTypes: [], brands: [] });
  const [openSections, setOpenSections] = useState({ rcHelicopterTypes: true, brands: true });
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const rcHelicopterProducts = loadedProducts.filter(p => 
          p.category === "Remote Control helicopters" || 
          p.subCategory === "Remote Control helicopters" ||
          p.for === "Remote Control helicopters"
        );
        setProducts(rcHelicopterProducts.length > 0 ? rcHelicopterProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(kidsProducts.filter(p => p.category === "Remote Control helicopters"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const clearAllFilters = () => {
    setFilters({ rcHelicopterTypes: [], brands: [] });
    setMaxPrice(5000);
  };
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      if (filters.rcHelicopterTypes.length > 0 && !filters.rcHelicopterTypes.includes(product.subCategory)) return false;
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600 mx-auto mb-3"></div><p className="text-gray-500 text-sm font-medium">Loading Remote Control Helicopters Collection...</p></div></div>;

  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="bg-gradient-to-r from-yellow-500 to-amber-500 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">🚁 Remote Control Helicopters</h1>
        <p className="mt-2 text-yellow-100 text-lg">Fly high with RC helicopters</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Remote Control Helicopters</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Remote Control Helicopters</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} RC helicopter items</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                ...
                No RC helicopter products found matching your criteria.
...
export default RemoteControlhelecopers;