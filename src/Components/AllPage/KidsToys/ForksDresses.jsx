// src/component/KidsFrockDresses.jsx
...
const FILTER_CONFIG = {
  // Dress Type
  type: [
    "Frock", "Princess Dress", "Party Wear Frock", "Casual Frock",
    "Flared Frock", "A-Line Frock", "Fit and Flare", "Tutu Dress",
    "Maxi Dress", "Mini Dress", "Cotton Frock", "Silk Frock",
    "Velvet Frock", "Net Frock", "Lace Frock", "Printed Frock",
    "Embroidered Frock", "Sequins Dress", "Birthday Dress", "Summer Dress",
    "Winter Dress", "Traditional Frock", "Western Dress", "School Dress"
  ],
  
  // Size (Kids Age)
  size: ["0-6M", "6-12M", "12-18M", "18-24M", "2-3Y", "3-4Y", "4-5Y", "5-6Y", "6-7Y", "7-8Y", "8-10Y", "10-12Y", "12-14Y"],
  
  // Color
  color: [
    "Pink", "Red", "White", "Cream", "Blue", "Purple", "Lavender", "Yellow",
    "Green", "Peach", "Orange", "Brown", "Black", "Maroon", "Multicolor"
  ],
  
  // Fabric
  fabric: ["Cotton", "Silk", "Velvet", "Net", "Lace", "Georgette", "Chiffon", "Polyester", "Satin", "Denim", "Linen", "Jersey"],
  
  // Sleeve Type
  sleeveType: ["Sleeveless", "Short Sleeve", "Half Sleeve", "Long Sleeve", "Puff Sleeve", "Bell Sleeve", "Cap Sleeve", "Flutter Sleeve"],
  
  // Length
  length: ["Mini", "Knee Length", "Below Knee", "Ankle Length", "Floor Length"],
  
  // Pattern
  pattern: ["Solid", "Printed", "Striped", "Polka Dots", "Floral", "Cartoon Print", "Unicorn Print", "Butterfly Print", "Animal Print", "Checkered"],
  
  // Features
  features: ["Pockets", "Bow Tie", "Tassels", "Frill", "Ruffle", "Lace Trim", "Elastic Waist", "Zip Closure", "Button Closure", "Adjustable Straps", "Breathable", "Soft Fabric"],
  
  // Brand
  brand: ["Gini & Jony", "Catmoss", "Lilliput", "Hopscotch", "Mothercare", "Babyhug", "Firstcry", "Marks & Spencer", "H&M", "Zara", "Biba", "W for Woman Kids"]
};

function ForksDresses() {
  ...
  const [filters, setFilters] = useState({ type: [], size: [], color: [], fabric: [], sleeveType: [], length: [], pattern: [], features: [], brand: [] });
  const [openSections, setOpenSections] = useState({ type: true, size: true, color: true, fabric: true, sleeveType: false, length: false, pattern: false, features: false, brand: false });
  ...
  const clearAllFilters = () => {
    setFilters({ type: [], size: [], color: [], fabric: [], sleeveType: [], length: [], pattern: [], features: [], brand: [] });
    setMaxPrice(3000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "frock" ||
    p.category?.toLowerCase() === "kids dress" ||
    p.name?.toLowerCase().includes("frock") ||
    p.name?.toLowerCase().includes("dress") ||
    (p.name?.toLowerCase().includes("kids") && p.name?.toLowerCase().includes("frock"))
  );
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm sticky top-20">
      ...
      <AccordionSection title=" Dress Type" sectionKey="type" items={FILTER_CONFIG.type} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-pink-600 to-rose-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Kids Frock & Dresses</h1>
      </div>
        <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Frock & Dresses</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Frock & Dresses</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Dresses Found</h3>
