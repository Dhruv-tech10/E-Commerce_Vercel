// src/component/BeautyFootCare.jsx
...
// ✨ FOOT CARE FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Scholl", "Himalaya", "Lotus", "VLCC", "Khadi", "Oriflame", "The Body Shop", "Biotique", "Patanjali", "Dr. Foot", "Aroma Magic", "Nykaa Naturals"],
  type: ["Foot Cream", "Foot Scrub", "Foot Mask", "Foot Soak", "Foot Spray", "Foot File/Rasp", "Foot Massager", "Foot Roller", "Crack Cream", "Foot Powder", "Anti-fungal Cream", "Callus Remover"],
  concern: ["Dry & Cracked Heels", "Foot Odor", "Pain Relief", "Tired Feet", "Swelling", "Dead Skin Removal", "Moisturizing", "Cooling Effect", "Exfoliation", "Anti-fungal"],
  formulation: ["Natural", "Herbal", "Ayurvedic", "Medicated", "Organic", "Chemical Free", "Paraben Free", "Dermatologist Tested"],
  skinType: ["All Skin Types", "Dry Skin", "Sensitive Skin", "Normal Skin", "Diabetic Friendly"],
  features: ["Quick Absorbing", "Non Greasy", "Long Lasting", "Cooling Sensation", "Warming Effect", "Waterproof", "Exfoliating", "Moisturizing"],
  size: ["15ml", "30ml", "50ml", "75ml", "100ml", "150ml", "200ml"]
};
...
function FootCare() {
  ...
  const footCareProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'foot care' || 
    p.name?.toLowerCase().includes('foot care')
  );
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], concern: [], formulation: [], skinType: [], features: [], size: [] });
    setMaxPrice(1500);
  };

  const filteredProducts = footCareProducts
    .filter((product) => {
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800"> Foot Care Filters</h2>
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
        ...
        <h1 className="text-3xl font-bold">Foot Care</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
       ...
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
        ...
        <span className="text-lg !font-medium text-[#E4921A]">Foot Care</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3"></h2>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700"> Refine Foot Care</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800"> Foot Care Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-amber-700 font-bold">{filteredProducts.length}</span> foot care products
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No foot care products match your filters.</p>
