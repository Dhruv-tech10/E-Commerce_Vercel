// src/component/BeautyVaseline.jsx
...
// 🫙 VASELINE FILTER CONFIGURATION
const FILTER_CONFIG = {
  type: ["Original Petroleum Jelly", "Lip Therapy", "Body Lotion", "Hand Cream", "Healing Jelly", "Aloe Vera", "Cocoa Butter", "Baby Care"],
  concern: ["Dry Skin", "Chapped Lips", "Healing", "Moisturizing", "Protection", "Baby Care", "Everyday Use"],
  formulation: ["Original", "Aloe Fresh", "Cocoa Glow", "Rosy Lips", "Vitamin E", "Sun Protection"],
  size: ["10ml", "20ml", "50ml", "100ml", "200ml", "250ml", "400ml"]
};
...
function Vaseline() {
  ...
  const vaselineProducts = products.filter((p) =>
    p.category?.toLowerCase() === 'vaseline' ||
    p.name?.toLowerCase().includes('vaseline') ||
    p.brand?.toLowerCase() === 'vaseline'
  );
  ...
  const clearAllFilters = () => {
    setFilters({ type: [], concern: [], formulation: [], size: [] });
    setMaxPrice(1000);
  };

  const filteredProducts = vaselineProducts
    .filter((product) => {
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button
        ...
        className="flex justify-between items-center w-full font-semibold text-gray-800 text-sm mb-2 hover:text-cyan-600 transition"
      >
        <span>{title}</span>
          ...
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
              <input
                ...
                className="w-3.5 h-3.5 !mr-2 rounded border-gray-300 text-cyan-600 focus:ring-cyan-500"
              />
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800">Vaseline Filters</h2>
        <button onClick={clearAllFilters} className="text-cyan-600 text-xs font-semibold hover:underline">Clear All</button>
      </div>
          ...
          <div className="flex gap-2 flex-wrap pt-1">
            {FILTER_CONFIG.size.map((size, idx) => (
              <button
                ...
                className={`border px-3 py-1.5 rounded-md text-xs font-medium transition ${filters.size.includes(size) ? 'bg-cyan-600 text-white border-cyan-600' : 'border-gray-200 hover:border-cyan-500 text-gray-600 bg-white'
                  }`}
              >
                {size}
              </button>
      ...
      <div className="mb-2 pb-3">
        ...
        <Slider
          ...
          sx={{
            color: "#0891b2",
            height: 4,
            '& .MuiSlider-thumb': { width: 16, height: 16, backgroundColor: '#0891b2' }
          }}
        />
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
        ...
        <h1 className="text-3xl font-bold">Vaseline</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Vaseline</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3">Vaseline</h2>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700">Refine Vaseline</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800"> Vaseline Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-cyan-600 font-bold">{filteredProducts.length}</span> vaseline products
              </span>
              <select
                value={sortBy}
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No vaseline products match your filters.</p>
                <button onClick={clearAllFilters} className="text-cyan-600 text-xs font-bold mt-2 underline">Reset Filters</button>
              </div>
            ) : (
