// src/component/BeautyWinterHealthcare.jsx
...
// ✨ WINTER HEALTHCARE FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Vicks", "Zandu", "Himalaya", "Dabur", "Patanjali", "Amrutanjan", "Moov", "Volini", "Relispray", "Kailash Jeevan", "Tiger Balm", "Sloan"],
  type: ["Vaporub", "Chest Rub", "Heating Patch", "Pain Relief Cream", "Nasal Inhaler", "Steam Inhaler", "Throat Lozenges", "Herbal Tea", "Immunity Booster", "Hot Water Bag", "Body Warmer", "Lip Balm"],
  concern: ["Cold & Cough", "Body Pain", "Headache", "Nasal Congestion", "Dry Skin", "Chapped Lips", "Low Immunity", "Joint Pain", "Muscle Stiffness", "Fever"],
  formulation: ["Ayurvedic", "Herbal", "Medicated", "Natural", "Organic", "Chemical Free", "Menthol Based", "Eucalyptus Based"],
  ageGroup: ["Babies", "Children", "Adults", "Elderly", "All Ages"],
  application: ["External Use", "Oral", "Inhalation", "Topical", "Steam"],
  features: ["Non-Greasy", "Fast Relief", "Long Lasting", "Pleasant Fragrance", "Easy to Apply", "Travel Friendly", "Doctor Recommended"],
  packSize: ["10ml", "25ml", "50ml", "100ml", "Pack of 2", "Pack of 5", "Family Pack"]
};

function WinterHealthcare() {
  ...
  const winterProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'winter healthcare' || 
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], concern: [], formulation: [], ageGroup: [], application: [], features: [], packSize: [] });
    setMaxPrice(1000);
  };
  ...
  const filteredProducts = winterProducts
    .filter((product) => {
      ...
      if (filters.application.length > 0 && !filters.application.includes(product.application)) return false;
          ...
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
              <input 
                ...
                className="w-3.5 h-3.5 !mr-2 rounded border-gray-300 text-amber-700 focus:ring-amber-500" 
              />
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800">Winter Healthcare Filters</h2>
      ...
      <AccordionSection title=" Age Group" sectionKey="ageGroup" items={FILTER_CONFIG.ageGroup} />
      <AccordionSection title=" Application" sectionKey="application" items={FILTER_CONFIG.application} />
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
        ...
        <h1 className="text-3xl font-bold">Winter Healthcare</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
       ...
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
        ...
        <span className="text-lg !font-medium text-[#E4921A]">Winter Healthcare</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3"></h2>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700">❄️ Refine Winter Healthcare</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800">❄️ Winter Healthcare Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-amber-700 font-bold">{filteredProducts.length}</span> winter care products
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No winter healthcare products match your filters.</p>
...
export default WinterHealthcare;