// src/component/BeautyFaceOil.jsx
...
// ✨ FACE OIL FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Biotique", "Khadi", "Mamaearth", "Plum", "Lotus", "Kama Ayurveda", "Forest Essentials", "Juicy Chemistry", "Nykaa Naturals", "The Body Shop"],
  type: ["Facial Oil", "Face Serum Oil", "Essential Oil", "Carrier Oil", "Night Oil", "Day Oil", "Anti-aging Oil", "Glow Oil"],
  oilType: ["Argan Oil", "Jojoba Oil", "Rosehip Oil", "Tea Tree Oil", "Almond Oil", "Coconut Oil", "Vitamin E Oil", "Squalane Oil"],
  skinType: ["All Skin Types", "Dry Skin", "Oily Skin", "Combination", "Sensitive", "Acne Prone"],
  concern: ["Anti-aging", "Glowing Skin", "Acne Control", "Dark Spots", "Hydration", "Skin Tightening", "Brightening", "Pigmentation"],
  formulation: ["Cold Pressed", "Organic", "Natural", "Non-Comedogenic", "Paraben Free", "Cruelty Free"],
  size: ["15ml", "20ml", "30ml", "50ml", "100ml"]
};
...
function FaceOil() {
  ...
  const faceOilProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'face oil' || 
    p.name?.toLowerCase().includes('face oil')
  );
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], oilType: [], skinType: [], concern: [], formulation: [], size: [] });
    setMaxPrice(3000);
  };

  const filteredProducts = faceOilProducts
    .filter((product) => {
      if (product.offerPrice > maxPrice) return false;
      if (filters.brands.length > 0 && !filters.brands.includes(product.brand)) return false;
      if (filters.type.length > 0 && !filters.type.includes(product.type)) return false;
      if (filters.oilType.length > 0 && !filters.oilType.includes(product.oilType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800">Face Oil Filters</h2>
      ...
      <AccordionSection title="Oil Type" sectionKey="oilType" items={FILTER_CONFIG.oilType} />
            ...
            {FILTER_CONFIG.size.map((size, idx) => (
              <button
                ...
                className={`border px-3 py-1.5 rounded-md text-xs font-medium transition ${
                  filters.size.includes(size) ? 'bg-amber-700 text-white border-amber-700' : 'border-gray-200 hover:border-amber-600 text-gray-600 bg-white'
                }`}
              >
                {size}
              </button>
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
        ...
        <h1 className="text-3xl font-bold">Face Oil</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
       ...
       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Face Oil</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3">Face Oil</h2>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700"> Refine Face Oil</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800"> Face Oil Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-amber-700 font-bold">{filteredProducts.length}</span> face oils
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No face oils match your filters.</p>
