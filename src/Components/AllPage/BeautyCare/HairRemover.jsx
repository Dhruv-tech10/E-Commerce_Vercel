// src/component/BeautyHairRemover.jsx
...
// ✂️ HAIR REMOVER FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Veet", "Anne French", "Fem", "Nair", "Sirona", "Bombay Shaving Company", "LetsShave", "Gillette"],
  type: ["Hair Removal Cream", "Wax Strips", "Hot Wax", "Cold Wax", "Epilator", "Razor", "Hair Removal Spray", "Face Hair Remover"],
  skinType: ["Normal", "Sensitive", "Dry", "Oily", "All Skin Types"],
  area: ["Full Body", "Face", "Arms & Legs", "Underarms", "Bikini Line", "Upper Lip"],
  formulation: ["Cream", "Gel", "Spray", "Powder", "Oil-based"],
  size: ["25g", "50g", "80g", "100g", "200g", "400g", "Pack of 2", "Pack of 3"]
};

function HairRemover() {
  ...
  const hairRemoverProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'hair remover' || 
    p.name?.toLowerCase().includes('hair remover')
  );
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], skinType: [], area: [], formulation: [], size: [] });
    setMaxPrice(5000);
  };

  const filteredProducts = hairRemoverProducts
    .filter((product) => {
      ...
      if (filters.area.length > 0 && !filters.area.includes(product.area)) return false;
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button 
        ...
        className="flex justify-between items-center w-full font-semibold text-gray-800 text-sm mb-2 hover:text-indigo-600 transition"
      >
        <span>{title}</span>
            ...
            {FILTER_CONFIG.size.map((size, idx) => (
              <button
                ...
                className={`border px-3 py-1.5 rounded-md text-xs font-medium transition ${
                  filters.size.includes(size) ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-200 hover:border-indigo-500 text-gray-600 bg-white'
                }`}
              >
                {size}
              </button>
      ...
      <div className="mb-2 pb-3">
        ...
        <Slider
          ...
          sx={{
            color: "#4f46e5",
            height: 4,
            '& .MuiSlider-thumb': { width: 16, height: 16, backgroundColor: '#4f46e5' }
          }}
        />
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <span className="text-lg !font-medium text-[#E4921A]">Hair Remover</span>
      </nav>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-indigo-600 font-bold">{filteredProducts.length}</span> hair removers
              </span>
              <select 
                value={sortBy} 
