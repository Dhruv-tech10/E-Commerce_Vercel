// src/component/BeautyEyeShadow.jsx
...
const FILTER_CONFIG = {
  brands: ["Lakme", "Maybelline", "Mac", "Nyx", "Huda Beauty", "Colorbar", "Swiss Beauty", "Revlon"],
  finish: ["Matte", "Shimmer", "Glitter", "Metallic", "Satin", "Pearl"],
  type: ["Single Eye Shadow", "Eye Shadow Palette", "Cream Eye Shadow", "Loose Powder", "Pressed Powder", "Eye Shadow Stick"],
  shade: ["Neutral", "Smokey", "Warm", "Cool", "Colorful", "Earth Tone", "Pastel"],
  size: ["1g", "2g", "3g", "5g", "9 Color", "12 Color", "18 Color", "24 Color"]
};
...
function EyeShadow() {
  ...
  const eyeShadowProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'eye shadow' || 
    p.name?.toLowerCase().includes('eye shadow')
  );
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], finish: [], type: [], shade: [], size: [] });
    setMaxPrice(5000);
  };

  const filteredProducts = eyeShadowProducts
    .filter((product) => {
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button 
        ...
        className="flex justify-between items-center w-full font-semibold text-gray-800 text-sm mb-2 hover:text-pink-600 transition"
      >
        <span>{title}</span>
            ...
            {FILTER_CONFIG.size.map((size, idx) => (
              <button
                ...
                className={`border px-3 py-1.5 rounded-md text-xs font-medium transition ${
                  filters.size.includes(size) ? 'bg-pink-600 text-white border-pink-600' : 'border-gray-200 hover:border-pink-500 text-gray-600 bg-white'
                }`}
              >
                {size}
              </button>
    ...
    <div className="bg-gray-50 min-h-screen pt-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Eye Shadows</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3">Eye Shadows</h2>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700">👁️ Refine Eye Shadows</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800">👁️ Eye Shadow Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-pink-600 font-bold">{filteredProducts.length}</span> eye shadows
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                <p className="text-gray-400 font-medium text-sm">No eye shadows match your filters.</p>
                <button onClick={clearAllFilters} className="text-pink-600 text-xs font-bold mt-2 underline">Reset Filters</button>
              </div>
            ) : (
