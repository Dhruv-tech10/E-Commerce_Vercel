// src/component/BeautyFoundation.jsx
...
// 💁‍♀️ FOUNDATION FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Lakme", "Maybelline", "Mac", "Nyx", "L'Oreal", "Revlon", "Colorbar", "Faces Canada", "Sugar", "Swiss Beauty"],
  type: ["Liquid Foundation", "Powder Foundation", "Cream Foundation", "Stick Foundation", "Cushion Foundation", "Serum Foundation", "Mousse Foundation"],
  finish: ["Matte", "Dewy", "Natural", "Luminous", "Satin", "Semi-Matte"],
  coverage: ["Sheer", "Light", "Medium", "Full", "Buildable"],
  shade: ["Ivory", "Fair", "Light Beige", "Medium", "Sand", "Warm Beige", "Honey", "Tan", "Espresso"],
  skinType: ["Oily", "Dry", "Combination", "Normal", "Sensitive"],
  spf: ["No SPF", "SPF 15", "SPF 20", "SPF 25", "SPF 30", "SPF 50"],
  size: ["15ml", "25ml", "30ml", "40ml", "50ml", "9g", "12g", "15g"]
};
...
function Foundation() {
  ...
  const foundationProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'foundation' || 
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], finish: [], coverage: [], shade: [], skinType: [], spf: [], size: [] });
    setMaxPrice(5000);
  };

  const filteredProducts = foundationProducts
    .filter((product) => {
      ...
      if (filters.coverage.length > 0 && !filters.coverage.includes(product.coverage)) return false;
      ...
      if (filters.spf.length > 0 && !filters.spf.includes(product.spf)) return false;
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button 
        ...
        className="flex justify-between items-center w-full font-semibold text-gray-800 text-sm mb-2 hover:text-amber-600 transition"
      >
        <span>{title}</span>
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800"> Foundation Filters</h2>
      ...
      <AccordionSection title=" Coverage" sectionKey="coverage" items={FILTER_CONFIG.coverage} />
      ...
      <AccordionSection title=" SPF" sectionKey="spf" items={FILTER_CONFIG.spf} />
            ...
            {FILTER_CONFIG.size.map((size, idx) => (
              <button
                ...
                className={`border px-3 py-1.5 rounded-md text-xs font-medium transition ${
                  filters.size.includes(size) ? 'bg-amber-600 text-white border-amber-600' : 'border-gray-200 hover:border-amber-500 text-gray-600 bg-white'
                }`}
              >
                {size}
              </button>
      ...
      <div className="mb-2 pb-3">
        ...
        <Slider
          ...
          sx={{
            color: "#d97706",
            height: 4,
            '& .MuiSlider-thumb': { width: 16, height: 16, backgroundColor: '#d97706' }
          }}
        />
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <span className="text-lg !font-medium text-[#E4921A]">Foundation</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3"></h2>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700"> Refine Foundations</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black !text-xs text-gray-800"> Foundation Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-amber-600 font-bold">{filteredProducts.length}</span> foundations
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No foundations match your filters.</p>
                <button onClick={clearAllFilters} className="text-amber-600 text-xs font-bold mt-2 underline">Reset Filters</button>
              </div>
            ) : (
