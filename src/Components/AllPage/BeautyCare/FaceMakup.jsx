// src/component/BeautyFaceMakeup.jsx
...
// 🎭 FACE MAKEUP FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Lakme", "Maybelline", "Mac", "Nyx", "L'Oreal", "Revlon", "Colorbar", "Sugar", "Swiss Beauty", "Faces Canada"],
  type: ["Foundation", "Concealer", "Compact", "Setting Powder", "BB Cream", "CC Cream", "Primer", "Contour", "Highlighter", "Blush"],
  finish: ["Matte", "Dewy", "Natural", "Glowy", "Satin"],
  coverage: ["Sheer", "Medium", "Full", "Buildable"],
  shade: ["Fair", "Light", "Medium", "Wheatish", "Dusky", "Dark"],
  skinType: ["Oily Skin", "Dry Skin", "Combination", "Normal", "Sensitive"]
};
...
function FaceMakup() {
  ...
  const faceMakeupProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'face makeup' || 
    p.name?.toLowerCase().includes('face makeup')
  );
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], finish: [], coverage: [], shade: [], skinType: [] });
    setMaxPrice(5000);
  };

  const filteredProducts = faceMakeupProducts
    .filter((product) => {
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button 
        ...
        className="flex justify-between items-center w-full font-semibold text-gray-800 text-sm mb-2 hover:text-purple-600 transition"
      >
        <span>{title}</span>
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800">Face Makeup Filters</h2>
      ...
      <div className="mb-2 pb-3">
        ...
        <Slider
          ...
          sx={{
            color: "#7c3aed",
            height: 4,
            '& .MuiSlider-thumb': { width: 16, height: 16, backgroundColor: '#7c3aed' }
          }}
        />
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
        ...
        <h1 className="text-3xl font-bold">Face Makeup</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Face Makeup</span>
        </nav>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700"> Refine Face Makeup</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800">Face Makeup Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-purple-600 font-bold">{filteredProducts.length}</span> face makeup products
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No face makeup products match your filters.</p>
                <button onClick={clearAllFilters} className="text-purple-600 text-xs font-bold mt-2 underline">Reset Filters</button>
              </div>
            ) : (
