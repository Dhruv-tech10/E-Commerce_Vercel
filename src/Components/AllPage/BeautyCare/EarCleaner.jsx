// src/component/BeautyEarCleaner.jsx
...
// ✨ EAR CLEANER FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Biotique", "HealthVit", "Dr. Trust", "Omron", "Beurer", "BPL", "Hicare", "3M", "Vandelay", "PureSense", "KareBay", "Medtech"],
  type: ["Ear Picks", "Ear Syringe", "Ear Wax Removal Kit", "Electric Ear Cleaner", "Manual Ear Cleaner", "Ear Vacuum", "Ear Irrigation Kit", "Smart Ear Cleaner"],
  material: ["Stainless Steel", "Plastic", "Silicone", "Bamboo", "Metal", "ABS Plastic", "Medical Grade Silicone", "Rubber"],
  powerSource: ["Manual", "Battery Operated", "USB Rechargeable", "Electric"],
  features: ["LED Light", "Water Resistant", "Portable", "Washable", "Ergonomic Design", "Multiple Tips", "Safety Stop", "Magnifying Mirror"],
  safetyFeature: ["Safety Tip", "Anti-Scratch", "Soft Tips", "Child Safe", "Doctor Recommended", "FDA Approved"],
  packSize: ["Single", "Pack of 2", "Pack of 3", "Pack of 5", "Pack of 10", "Family Pack"]
};
...
function EarCleaner() {
  ...
  const earCleanerProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'ear cleaner' || 
    p.name?.toLowerCase().includes('ear cleaner')
  );
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], material: [], powerSource: [], features: [], safetyFeature: [], packSize: [] });
    setMaxPrice(2000);
  };

  const filteredProducts = earCleanerProducts
    .filter((product) => {
      ...
      if (filters.safetyFeature.length > 0 && !product.safetyFeature?.some((s) => filters.safetyFeature.includes(s))) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800">Ear Cleaner Filters</h2>
      ...
      <AccordionSection title=" Safety Features" sectionKey="safetyFeature" items={FILTER_CONFIG.safetyFeature} />
    ...
    <div className="bg-gray-50 min-h-screen pt-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Ear Cleaner</span>
        </nav>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700">Refine Ear Cleaner</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800"> Ear Cleaner Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-amber-700 font-bold">{filteredProducts.length}</span> ear cleaners
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No ear cleaners match your filters.</p>
