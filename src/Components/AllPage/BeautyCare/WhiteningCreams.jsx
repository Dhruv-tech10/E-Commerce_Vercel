// src/component/BeautyWhiteCreams.jsx
...
// ⚪ WHITE CREAMS FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Ponds", "Fair & Lovely", "Lotus", "Biotique", "Himalaya", "Olay", "L'Oreal", "Nivea", "Lakme", "Mamaearth"],
  type: ["Whitening Cream", "Day Cream", "Night Cream", "BB Cream", "DD Cream", "Fairness Cream", "Anti-spot Cream", "Brightening Cream"],
  skinType: ["Normal", "Oily", "Dry", "Combination", "Sensitive", "All Skin Types"],
  concern: ["Skin Whitening", "Dark Spots", "Uneven Tone", "Dullness", "Pigmentation", "Sun Damage", "Acne Marks"],
  formulation: ["Vitamin C", "Glutathione", "Kojic Acid", "Natural", "Ayurvedic", "With SPF", "Gel Based", "Cream Based"],
  timeOfUse: ["Day", "Night", "Day & Night"],
  size: ["15g", "20g", "25g", "30g", "40g", "50g", "100g"]
};

function WhiteningCreams() {
  ...
  const whiteCreamsProducts = products.filter((p) => 
    p.category?.toLowerCase() === 'white creams' || 
    p.name?.toLowerCase().includes('white cream') ||
    p.name?.toLowerCase().includes('whitening') ||
    p.name?.toLowerCase().includes('fairness')
  );
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], skinType: [], concern: [], formulation: [], timeOfUse: [], size: [] });
    setMaxPrice(2000);
  };

  const filteredProducts = whiteCreamsProducts
    .filter((product) => {
      ...
      if (filters.timeOfUse.length > 0 && !filters.timeOfUse.includes(product.timeOfUse)) return false;
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button 
        ...
        className="flex justify-between items-center w-full font-semibold text-gray-800 text-sm mb-2 hover:text-gray-700 transition"
      >
        <span>{title}</span>
          ...
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
              <input 
                ...
                className="w-3.5 h-3.5 !mr-2 rounded border-gray-300 text-gray-700 focus:ring-gray-500" 
              />
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="!font-bold !text-base text-gray-800">White Cream Filters</h2>
        <button onClick={clearAllFilters} className="text-gray-700 text-xs font-semibold hover:underline">Clear All</button>
      </div>

      <AccordionSection title=" Brand" sectionKey="brands" items={FILTER_CONFIG.brands} />
      <AccordionSection title=" Cream Type" sectionKey="type" items={FILTER_CONFIG.type} />
      ...
      <AccordionSection title=" Key Ingredient" sectionKey="formulation" items={FILTER_CONFIG.formulation} />
      <AccordionSection title=" Time of Use" sectionKey="timeOfUse" items={FILTER_CONFIG.timeOfUse} />
            ...
            {FILTER_CONFIG.size.map((size, idx) => (
              <button
                ...
                className={`border px-3 py-1.5 rounded-md text-xs font-medium transition ${
                  filters.size.includes(size) ? 'bg-gray-700 text-white border-gray-700' : 'border-gray-200 hover:border-gray-500 text-gray-600 bg-white'
                }`}
              >
                {size}
              </button>
      ...
      <div className="mb-2 pb-3">
        ...
        <Slider
          ...
          sx={{
            color: "#374151",
            height: 4,
            '& .MuiSlider-thumb': { width: 16, height: 16, backgroundColor: '#374151' }
          }}
        />
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
        ...
        <h1 className="text-3xl font-bold">White Creams</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <span className="text-lg !font-medium text-[#E4921A]">White Creams</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3">White Creams</h2>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700"> Refine White Creams</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800"> White Cream Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-gray-700 font-bold">{filteredProducts.length}</span> white creams
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No white creams match your filters.</p>
                <button onClick={clearAllFilters} className="text-gray-700 text-xs font-bold mt-2 underline">Reset Filters</button>
              </div>
            ) : (
...
export default WhiteningCreams;