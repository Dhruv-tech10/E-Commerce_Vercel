...
// 🧴 CREAM FILTER CONFIGURATION
const FILTER_CONFIG = {
  brands: ["Ponds", "Nivea", "Lotus", "Himalaya", "Olay", "L'Oreal", "Garnier", "Biotique", "Mamaearth", "Plum"],
  type: ["Face Cream", "Hand Cream", "Body Cream", "Eye Cream", "Neck Cream", "Multipurpose Cream"],
  concern: ["Whitening", "Anti-aging", "Moisturizing", "Acne Control", "Dark Spots", "Wrinkles", "Sun Protection", "Brightening"],
  skinType: ["Normal", "Dry", "Oily", "Combination", "Sensitive", "All Skin Types"],
  formulation: ["Cream", "Gel Cream", "Lotion", "Rich Cream", "Light Cream"],
  timeOfUse: ["Day Cream", "Night Cream", "Day & Night"],
  size: ["10g", "20g", "30g", "50g", "100g", "150g", "200g"]
};
...
function Cream() {
  ...
  const clearAllFilters = () => {
    setFilters({ brands: [], type: [], concern: [], skinType: [], formulation: [], timeOfUse: [], size: [] });
    setMaxPrice(5000);
  };

  const filteredProducts = creamProducts
    .filter((product) => {
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button 
        ...
        className="flex justify-between items-center w-full font-semibold text-gray-800 text-sm mb-2 hover:text-emerald-600 transition"
      >
        <span>{title}</span>
          ...
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
              <input 
                ...
                className="w-3.5 h-3.5 !mr-2 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" 
              />
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      <div className="flex justify-between items-center border-b pb-3">
        <h2 className="font-bold text-base text-gray-800">Cream Filters</h2>
        <button onClick={clearAllFilters} className="text-emerald-600 text-xs font-semibold hover:underline">Clear All</button>
      </div>
            ...
            {FILTER_CONFIG.size.map((size, idx) => (
              <button
                ...
                className={`border px-3 py-1.5 rounded-md text-xs font-medium transition ${
                  filters.size.includes(size) ? 'bg-emerald-600 text-white border-emerald-600' : 'border-gray-200 hover:border-emerald-500 text-gray-600 bg-white'
                }`}
              >
                {size}
              </button>
      ...
      <div className="mb-2 pb-3">
        ...
        <Slider
          ...
          sx={{
            color: "#059669",
            height: 4,
            '& .MuiSlider-thumb': { width: 16, height: 16, backgroundColor: '#059669' }
          }}
        />
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
        ...
        <h1 className="text-3xl font-bold">Cream</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3">
          ...
          <span className="text-lg !font-medium text-[#E4921A]">Cream</span>
        </nav>
        ...
        <div className="md:hidden mb-4">
          ...
          >
            <span className="font-bold text-sm text-gray-700">Refine Creams</span>
            <Menu size={18} />
          </button>
        </div>
          ...
          <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={() => setMobileFiltersOpen(false)}>
            <div className="absolute top-0 left-0 w-80 h-full bg-white overflow-y-auto p-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center border-b pb-3 mb-2">
                <h2 className="font-black text-gray-800">Cream Filters</h2>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">
                Found <span className="text-emerald-600 font-bold">{filteredProducts.length}</span> creams
              </span>
              <select 
                value={sortBy} 
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-16 text-center shadow-sm">
                ...
                <p className="text-gray-400 font-medium text-sm">No creams match your filters.</p>
                <button onClick={clearAllFilters} className="text-emerald-600 text-xs font-bold mt-2 underline">Reset Filters</button>
              </div>
            ) : (
