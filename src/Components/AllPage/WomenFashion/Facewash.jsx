// src/component/WomenFacewash.jsx
...
// 🧴 FACE WASH FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  skinType: ["Oily Skin", "Dry Skin", "Combination Skin", "Normal Skin", "Sensitive Skin", "Acne Prone"],
  keyIngredients: ["Neem", "Tea Tree", "Charcoal", "Aloe Vera", "Honey", "Turmeric", "Salicylic Acid", "Vitamin C"],
  benefits: ["Anti-Acne", "Brightening", "Deep Cleansing", "Hydrating", "Exfoliating", "Oil Control", "Anti-Aging"],
  brands: ["Himalaya", "Lakme", "Neutrogena", "Clean & Clear", "Cetaphil", "Plum", "Mamaearth", "The Derma Co"]
};

const SIDEBAR_SECTIONS = [
  { title: " Skin Type", key: "skinType" },
  { title: " Key Ingredients", key: "keyIngredients" },
...
function Facewash() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const facewashProducts = loadedProducts.filter(p => 
          p.category === "Face Wash" || 
          p.subCategory === "Facewash" ||
          p.productType === "Face Wash" ||
          p.name?.toLowerCase().includes("face wash")
        );
        setProducts(facewashProducts.length > 0 ? facewashProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(beautyProducts.filter(p => p.category === "Face Wash"));
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm font-medium">Loading Face Wash Collection...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <div className="bg-gradient-to-r from-green-500 to-teal-600 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">Face Wash</h1>
        <p className="mt-2 text-green-100 text-lg">Gentle cleansing for radiant skin</p>
      </div>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} face wash products</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                No face wash products found matching your criteria.
