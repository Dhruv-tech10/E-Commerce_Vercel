...
function Palazzo() {
  ...
  const palazzosProducts = products
    .filter((p) => p.category?.trim().toLowerCase() === "palazzos")
    .filter((product) => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            ...
            <span className="text-lg !font-medium text-[#E4921A]">Palazzos</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Palazzos</h2>
      <div className="max-w-7xl mx-auto flex gap-6 p-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold mb-6">Women's Palazzos</h1>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {palazzosProducts.length > 0 ? (
              palazzosProducts.map((product) => (
                <ProductCard key={product.id} product={product} />
              ))
            ) : (
              <p className="text-gray-500">No Palazzos Found</p>
            )}
          </div>
        </div>
