// src/component/WomenTShirts.jsx
...
// 👚 WOMEN T-SHIRTS FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
...
const SIDEBAR_SECTIONS = [
  { title: "🧵 Fabric", key: "fabrics" },
  { title: "📦 Combo", key: "combos" },
];
...
function TShirt() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const tshirtProducts = loadedProducts.filter(p => 
          p.category === "Women T-Shirts" || 
          p.subCategory === "Women T-Shirts" ||
          p.for === "Women T-Shirts"
        );
        setProducts(tshirtProducts.length > 0 ? tshirtProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(womenProducts.filter(p => p.category === "Women T-Shirts"));
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600 mx-auto mb-3"></div><p className="text-gray-500 text-sm font-medium">Loading Women T-Shirts Collection...</p></div></div>;

  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="bg-gradient-to-r from-pink-500 to-rose-500 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">👚 Women T-Shirts</h1>
        <p className="mt-2 text-pink-100 text-lg">Stylish t-shirts for women</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">T-Shirts</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">T-Shirts</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} t-shirt items</span>
