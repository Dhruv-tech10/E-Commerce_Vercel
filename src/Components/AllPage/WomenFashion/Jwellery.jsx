// src/component/WomenJewellerySets.jsx
...
function Jwellery() {
  ...
  const FILTER_CONFIG = {
    types: ["Necklace Sets", "Earring Sets", "Bangle Sets", "Ring Sets", "Complete Sets"],
  ...
  const clearAllFilters = () => {
    setFilters({ types: [], materials: [], sizes: [] });
    setMaxPrice(5000);
  };

  const jewellerySetsProducts = products
    .filter((p) => p.category?.trim().toLowerCase() === "jewellery sets")
    .filter((product) => {
      if (product.offerPrice > maxPrice) return false;
      if (filters.types.length > 0 && !filters.types.includes(product.subCategory)) return false;
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Jewellery Collection</h1>
        <p className="mt-2 text-white/80 text-sm">Timeless Elegance for Every Occasion</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Jewellery</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Jewellery Sets</h2>
      <div className="max-w-7xl mx-auto flex gap-6 p-4">
        ...
        <div className="flex-1">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {jewellerySetsProducts.length > 0 ? (
              jewellerySetsProducts.map((product) => (
                <ProductCard key={product.id} product={product} />
              ))
            ) : (
              <p className="text-gray-500">No Jewellery Sets Found</p>
            )}
          </div>
        </div>
