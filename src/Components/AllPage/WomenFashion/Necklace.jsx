...
function Necklace() {
  ...
  const FILTER_CONFIG = {
    types: ["Pendant Necklaces", "Choker Necklaces", "Statement Necklaces", "Layered Necklaces", "Charm Necklaces"],
  ...
  const necklacesProducts = products
    .filter((p) => p.category?.trim().toLowerCase() === "necklaces")
    .filter((product) => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Necklaces Collection</h1>
        <p className="mt-2 text-white/80 text-sm">Timeless Elegance for Every Occasion</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Necklaces</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Necklaces</h2>
        ...
        <div className="flex-1">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {necklacesProducts.length > 0 ? (
              ...
              <p className="text-gray-500">No Necklaces Found</p>
            )}
          </div>
        </div>
