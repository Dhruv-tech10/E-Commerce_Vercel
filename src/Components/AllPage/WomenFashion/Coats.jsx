// src/component/WomenCoats.jsx
import { useState, useEffect } from "react";
import { Slider } from "@mui/material";
import ProductCard from "../ProductCard";
import { Menu, X, ChevronDown, ChevronUp } from "lucide-react";
import { womenProducts, loadAllWomenProducts } from "../ProductsData";
import { Link } from 'react-router-dom';
// 👘 WOMEN COATS FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  coatType: ["Winter Coats", "Denim Jackets", "Leather Jackets", "Blazers", "Cardigans", "Parkas", "Puffer Jackets", "Trench Coats"],
  fabric: ["Cotton", "Wool", "Polyester", "Denim", "Leather", "Velvet", "Fleece", "Cashmere"],
  pattern: ["Solid", "Striped", "Checked", "Printed", "Floral", "Polka Dot", "Abstract"],
  length: ["Short", "Medium", "Long", "Cropped"],
  brands: ["Zara", "H&M", "Forever 21", "Levi's", "Allen Solly", "Van Heusen", "Only", "Vero Moda"]
};

// Sidebar configuration
const SIDEBAR_SECTIONS = [
  { title: " Coat Type", key: "coatType" },
...
function Coats() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const loadedProducts = await loadAllWomenProducts();
        const coatsProducts = loadedProducts.filter(p => 
          p.category === "Coats" || 
          p.category === "Jackets" ||
          p.subCategory === "Women Coats" ||
          p.productType === "Coats"
        );
        setProducts(coatsProducts.length > 0 ? coatsProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(womenProducts.filter(p => p.category === "Coats"));
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm font-medium">Loading Coats Collection...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <div className="bg-gradient-to-r from-pink-500 to-rose-600 text-white text-center py-12 px-4">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide">Women's Coats & Jackets</h1>
        <p className="mt-2 text-pink-100 text-lg">Stay warm & stylish this season</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Coats</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Coats</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} coats & jackets</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                No coats & jackets found matching your criteria.
