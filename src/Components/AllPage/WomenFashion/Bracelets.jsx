...
function Bracelets() {
  ...
  const FILTER_CONFIG = {
    types: ["Charm Bracelets", "Bangles", "Cuff Bracelets", "Chain Bracelets", "Beaded Bracelets"],
    materials: ["Gold", "Silver", "Rose Gold", "Stainless Steel", "Alloy", "Beads", "Leather"],
  ...
  const clearAllFilters = () => {
    setFilters({ materials: [], sizes: [], types: [] });
    setMaxPrice(5000);
  };

  const braceletsProducts = products
    .filter((p) => p.subCategory?.toLowerCase() === "bracelets")
    .filter((product) => {
      if (product.offerPrice > maxPrice) return false;
      if (filters.types.length > 0 && !filters.types.includes(product.productType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
      ...
      <AccordionSection title=" Type" sectionKey="types" items={FILTER_CONFIG.types} />
        ...
        <div className="px-2 py-3">
          <Slider 
            ...
            sx={{
              color: "#ea580c",
              height: 6,
              '& .MuiSlider-thumb': { width: 20, height: 20, backgroundColor: '#ea580c' },
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            ...
            <span className="text-lg !font-medium text-[#E4921A]">Bracelets</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Bracelets</h2>
      <div className="max-w-7xl mx-auto flex gap-6 p-4">
        <div className="w-72 hidden md:block"><FilterSidebar /></div>
        <div className="flex-1">
          <h1 className="text-3xl font-bold mb-6">Bracelets Collection</h1>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {braceletsProducts.length > 0 ? (
              braceletsProducts.map((product) => <ProductCard key={product.id} product={product} />)
            ) : (
