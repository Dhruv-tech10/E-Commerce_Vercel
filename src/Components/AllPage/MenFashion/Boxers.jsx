...
const FILTER_CONFIG = {
  // Fabric Filter
  fabric: [
    "Cotton", "Cotton Blend", "Microfiber", "Modal", "Bamboo",
    "Lycra", "Spandex", "Nylon", "Polyester", "Silk",
    "Organic Cotton", "Supima Cotton", "Egyptian Cotton",
    "Jersey", "French Terry", "Mesh", "Air Mesh"
  ],
  
  // Combo / Quantity Filter
  combo: [
    "Pack of 1", "Pack of 2", "Pack of 3", "Pack of 4", 
    "Pack of 5", "Pack of 6", "Pack of 7", "Pack of 8",
    "Pack of 10", "Pack of 12", "Single"
  ]
};
...
function Boxers() {
  ...
  // ✅ ONLY BOXERS PRODUCTS
  let filteredProducts = products.filter(
    (p) => p.category?.trim().toLowerCase() === "boxers"
  );
    ...
    <div className="bg-white p-4 rounded-xl shadow-sm h-full min-h-[calc(100vh-200px)]">
      ...
      <div className="mb-4 pt-2">
        ...
        <div className="flex justify-between text-[10px] text-gray-400 mt-1">
          <span>₹0</span><span>₹625</span><span>₹1250</span><span>₹1875</span><span>₹2500</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Men's Boxers Collection</h1>
        <p className="mt-2 text-white/80 text-sm">Ultimate Comfort & Breathability for Daily Wear</p>
      </div>
      
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7 flex-shrink-0">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Men's Boxers</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7 flex-shrink-0">Men's Boxers</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <p className="text-gray-500 text-sm">No Boxers found matching your filters.</p>
                <button 
