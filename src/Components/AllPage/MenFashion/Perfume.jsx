...
const FILTER_CONFIG = {
  fragranceFamily: [
    "Woody", "Oriental", "Fresh", "Citrus", "Aromatic", 
    "Leather", "Spicy", "Aquatic", "Green", "Fougere",
    "Musk", "Amber", "Vanilla", "Tobacco", "Oud"
  ],
  concentration: [
    "Eau de Parfum", "Eau de Toilette", "Eau de Cologne", 
    "Perfume Oil", "Attar", "Body Spray", "Deodorant"
  ],
  notes: [
    "Sandalwood", "Vetiver", "Bergamot", "Lemon", "Orange",
    "Patchouli", "Cedarwood", "Musk", "Amber", "Vanilla",
    "Tobacco", "Leather", "Cardamom", "Pepper", "Cinnamon"
  ],
  occasion: [
    "Daily Wear", "Party", "Office", "Romantic", "Wedding", 
    "Summer", "Winter", "Evening", "Night Out", "Formal"
  ],
  brands: [
    "Davidoff", "Calvin Klein", "Hugo Boss", "Armani", "Versace",
    "Ajmal", "Bellavita", "Wild Stone", "Park Avenue", "Fogg",
    "Nivea", "Adidas", "Puma", "Rasasi", "Lattafa"
  ]
};
...
function MenPerfume() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category === "Men Perfumes" || p.subCategory === "Perfume" || p.name?.toLowerCase().includes("perfume") || p.name?.toLowerCase().includes("deodorant")
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.notes.length > 0 && !filters.notes.includes(product.notes)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title="Fragrance Family" sectionKey="fragranceFamily" items={FILTER_CONFIG.fragranceFamily} />
      <AccordionSection title="Concentration" sectionKey="concentration" items={FILTER_CONFIG.concentration} />
      <AccordionSection title="Key Notes" sectionKey="notes" items={FILTER_CONFIG.notes} />
...
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            ...
            <span className="text-lg !font-medium text-[#E4921A]">Men's Perfume</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Men's Perfume</h2>
          ...
          <div className="flex-1">
            <div className="bg-white rounded-xl shadow-sm p-3 mb-4 flex justify-between items-center flex-wrap gap-2">
              <div className="text-xs text-gray-500">
                Showing <span className="font-semibold text-gray-700">{filteredProducts.length}</span> perfumes
              </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                <p className="text-gray-500 text-sm">No Perfumes found matching your filters.</p>
                <button 
