...
const FILTER_CONFIG = {
  skinType: [
    ...
    "Sensitive Skin", "Acne Prone", "Dull Skin", "Uneven Texture"
  ],
  keyIngredients: [
    "Neem", "Tea Tree", "Charcoal", "Aloe Vera", "Honey", 
    "Turmeric", "Salicylic Acid", "Vitamin C", "Niacinamide",
    "Coffee", "Multani Mitti", "Clay", "Cucumber", "Lemon"
  ],
  benefits: [
    "Anti-Acne", "Oil Control", "Deep Cleansing", "Brightening", 
    "Exfoliating", "Hydrating", "Anti-Aging", "Tan Removal",
    "Pore Reduction", "Glow Boost", "Mattifying"
  ],
  brands: [
    "Nivea Men", "Garnier Men", "Himalaya", "Lakme", 
    "Mamaearth", "Plum", "Cetaphil", "Neutrogena",
    "The Man Company", "Bombay Shaving Company", "Mcaffeine", "Wow"
  ]
};

function MenFacewash() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category === "Men Face Wash" || p.subCategory === "Face Wash" || p.name?.toLowerCase().includes("face wash")
  );

  filteredProducts = filteredProducts.filter((product) => {
    if (product.offerPrice > maxPrice) return false;
    if (filters.skinType.length > 0 && !filters.skinType.includes(product.skinType)) return false;
    if (filters.keyIngredients.length > 0 && !filters.keyIngredients.includes(product.keyIngredient)) return false;
    if (filters.benefits.length > 0 && !filters.benefits.includes(product.benefit)) return false;
  ...
  {/* ચેન્જ અહીં કર્યો છે: space-y-2 ની સાથે grid grid-cols-1 આપ્યું છે જેથી ફરજિયાત બધું એક લાઈનમાં જ આવે */}
      ...
      {openSections[sectionKey] && (
        <div className="grid grid-cols-1 gap-1.5 max-h-60 overflow-y-auto pr-1">
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2.5 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition-colors w-full">
              <input 
                type="checkbox" 
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title="Key Ingredients" sectionKey="keyIngredients" items={FILTER_CONFIG.keyIngredients} />
      <AccordionSection title="Benefits" sectionKey="benefits" items={FILTER_CONFIG.benefits} />
      ...
      <div className="mb-4 pt-2">
        ...
        <div className="flex justify-between text-[10px] text-gray-400 mt-1">
          <span>₹0</span><span>₹375</span><span>₹750</span><span>₹1125</span><span>₹1500</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        ...
        <span className="text-lg !font-medium text-[#E4921A]">Men's Face Wash</span>
      </nav>
      
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Men's Face Wash</h2>
          ...
          <div className="flex-1">
            <div className="bg-white rounded-xl shadow-sm p-3 mb-4 flex justify-between items-center flex-wrap gap-2">
              <div className="text-xs text-gray-500">
                Showing <span className="font-semibold text-gray-700">{filteredProducts.length}</span> face wash products
              </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                <p className="text-gray-500 text-sm">No Face Wash products found matching your filters.</p>
                <button 
