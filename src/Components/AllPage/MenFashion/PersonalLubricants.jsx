// src/component/MenPersonalLubricants.jsx
...
const FILTER_CONFIG = {
  lubricantType: [
    ...
    "Aloe Based", "Natural", "Organic", "Flavored",
    "Warming", "Cooling", "Tingling", "Long Lasting"
  ],
  features: [
    "Paraben Free", "Glycerin Free", "Fragrance Free", "Hypoallergenic",
    "Vegan", "Cruelty Free", "Edible", "Stain Free",
    "Latex Safe", "Non Sticky", "Easy Cleanup", "Long Lasting"
  ],
  flavor: [
    "Unflavored", "Strawberry", "Vanilla", "Chocolate", 
    "Mint", "Cherry", "Orange", "Grape", "Berry Mix"
  ],
  brands: [
    "Durex", "Kama Sutra", "Manforce", "Skore", "Mood",
    "Sensual", "Bold Care", "MyMuse", "That Sassy Thing", "Sirona"
  ]
};

function PersonalLubricants() {
  ...
  let filteredProducts = products.filter(
    (p) => p.category === "Personal Lubricants" || p.subCategory === "Lubricants" || p.name?.toLowerCase().includes("lubricant") || p.name?.toLowerCase().includes("lube")
  );

  filteredProducts = filteredProducts.filter((product) => {
    if (product.offerPrice > maxPrice) return false;
    if (filters.lubricantType.length > 0 && !filters.lubricantType.includes(product.lubricantType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title="Lubricant Type" sectionKey="lubricantType" items={FILTER_CONFIG.lubricantType} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-rose-700 to-pink-800 text-white text-center py-8">
        <h1 className="text-3xl font-bold">Personal Lubricants</h1>
        <p className="mt-2 text-white/80 text-sm">For a Smooth & Comfortable Experience</p>
      </div>
 <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Personal Lubricants</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Personal Lubricants</h2>
          ...
          <div className="flex-1">
            <div className="bg-white rounded-xl shadow-sm p-3 mb-4 flex justify-between items-center flex-wrap gap-2">
              <div className="text-xs text-gray-500">
                Showing <span className="font-semibold text-gray-700">{filteredProducts.length}</span> lubricants
              </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                <p className="text-gray-500 text-sm">No Lubricants found matching your filters.</p>
                <button 
...
export default PersonalLubricants;