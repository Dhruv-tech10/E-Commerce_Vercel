// src/component/MenJewellery.jsx
...
const FILTER_CONFIG = {
  jewelleryType: [
    "Rings", "Chains", "Necklaces", "Bracelets", "Pendants", 
    "Cufflinks", "Tie Pins", "Lapel Pins", "Money Clips",
    "Silver Rings", "Gold Chains", "Steel Bracelets", "Leather Bracelets"
  ],
  material: [
    "Silver", "Stainless Steel", "Gold Plated", "Titanium", 
    "Tungsten", "Ceramic", "Leather", "Copper", "Brass",
    "Rose Gold", "White Gold", "Platinum"
  ],
  metalType: [
    "Sterling Silver", "925 Silver", "18K Gold Plated", "22K Gold", 
    "Gold Tone", "Silver Tone", "Black Steel", "Matt Finish", "Glossy Finish"
  ],
  style: [
    "Modern", "Classic", "Traditional", "Minimalist", "Statement", 
    "Casual", "Formal", "Party Wear", "Wedding", "Daily Wear"
  ]
};

function MenJewellery() {
  ...
  const clearAllFilters = () => {
    setFilters({ jewelleryType: [], material: [], metalType: [], style: [] });
    setMaxPrice(3000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(
    (p) => p.category === "Men Jewellery" || p.subCategory === "Jewellery" || p.name?.toLowerCase().includes("jewellery") || p.name?.toLowerCase().includes("ring") || p.name?.toLowerCase().includes("chain")
  );

  filteredProducts = filteredProducts.filter((product) => {
    if (product.offerPrice > maxPrice) return false;
    if (filters.jewelleryType.length > 0 && !filters.jewelleryType.includes(product.jewelleryType)) return false;
    if (filters.material.length > 0 && !filters.material.includes(product.material)) return false;
    if (filters.metalType.length > 0 && !filters.metalType.includes(product.metalType)) return false;
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      ...
      {openSections[sectionKey] && (<div className="grid grid-cols-1 gap-1.5 max-h-60 overflow-y-auto pr-1">{items.map((item, index) => (<label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded"><input type="checkbox" checked={filters[sectionKey]?.includes(item)} onChange={() => handleFilterToggle(sectionKey, item)} className="w-3.5 h-3.5 !mr-2 rounded border-gray-300 text-orange-600 focus:ring-orange-500" /><span className="text-gray-600 !text-sm">{item}</span></label>))}</div>)}
    </div>
  );
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title="Jewellery Type" sectionKey="jewelleryType" items={FILTER_CONFIG.jewelleryType} />
      <AccordionSection title="Material" sectionKey="material" items={FILTER_CONFIG.material} />
      <AccordionSection title="Metal Type" sectionKey="metalType" items={FILTER_CONFIG.metalType} />
      <AccordionSection title="Style" sectionKey="style" items={FILTER_CONFIG.style} />
      <div className="mb-4 pt-2"><h2 className="font-bold text-gray-800 text-sm mb-2">Max Price: ₹{maxPrice}</h2><div className="px-1"><Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} valueLabelDisplay="auto" min={0} max={5000} step={100} sx={{ color: "#ea580c", height: 4, '& .MuiSlider-thumb': { width: 18, height: 18, backgroundColor: '#ea580c' }, '& .MuiSlider-track': { backgroundColor: '#ea580c' }, '& .MuiSlider-rail': { backgroundColor: '#d1d5db' } }} /></div><div className="flex justify-between text-[10px] text-gray-400 mt-1"><span>₹0</span><span>₹1250</span><span>₹2500</span><span>₹3750</span><span>₹5000</span></div></div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Men's Jewellery</h1><p className="mt-2 text-white/80 text-sm">Elevate Your Style</p></div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Men's Jewellery</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Men's Jewellery</h2>
