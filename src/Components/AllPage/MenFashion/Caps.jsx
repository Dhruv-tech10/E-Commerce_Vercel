...
const FILTER_CONFIG = {
  capType: [
    "Baseball Caps", "Snapback Caps", "Dad Hats", "Trucker Caps", 
    "Flat Brim Caps", "Curved Brim Caps", "Fitted Caps", "Adjustable Caps",
    "Golf Caps", "Sports Caps", "Cotton Caps", "Mesh Caps"
  ],
  material: [
    "Cotton", "Polyester", "Wool", "Denim", "Twill", 
    "Canvas", "Mesh", "Acrylic", "Nylon", "Leather"
  ],
  color: [
    "Black", "White", "Navy Blue", "Red", "Blue", 
    "Green", "Grey", "Brown", "Khaki", "Beige", "Multicolor"
  ],
  pattern: [
    "Solid", "Printed", "Embroidered", "Logo", "Striped", 
    "Camouflage", "Color Block", "Graphic", "Text Print"
  ]
};
...
function MenCaps() {
  ...
  const clearAllFilters = () => {
    setFilters({ capType: [], material: [], color: [], pattern: [] });
    setMaxPrice(1500);
    setSortBy("default");
  };

  let filteredProducts = products.filter(
    (p) => p.category === "Men Caps" || p.subCategory === "Caps" || p.name?.toLowerCase().includes("cap")
  );

  filteredProducts = filteredProducts.filter((product) => {
    if (product.offerPrice > maxPrice) return false;
    if (filters.capType.length > 0 && !filters.capType.includes(product.capType)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title="Cap Type" sectionKey="capType" items={FILTER_CONFIG.capType} />
      ...
      <div className="mb-4 pt-2"><h2 className="font-bold text-gray-800 text-sm mb-2">Max Price: ₹{maxPrice}</h2><div className="px-1"><Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} valueLabelDisplay="auto" min={0} max={3000} step={50} sx={{ color: "#ea580c", height: 4, '& .MuiSlider-thumb': { width: 18, height: 18, backgroundColor: '#ea580c' }, '& .MuiSlider-track': { backgroundColor: '#ea580c' }, '& .MuiSlider-rail': { backgroundColor: '#d1d5db' } }} /></div><div className="flex justify-between text-[10px] text-gray-400 mt-1"><span>₹0</span><span>₹750</span><span>₹1500</span><span>₹2250</span><span>₹3000</span></div></div>
    </div>
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Men's Caps Collection</h1><p className="mt-2 text-white/80 text-sm">Complete Your Look</p></div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Men's Caps</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Men's Caps</h2>
