...
const FILTER_CONFIG = {
  type: ["Massage Oil", "Aromatherapy Oil", "Essential Oil Blend", "Sensual Massage Oil", "Warming Oil", "Cooling Oil", "Natural Oil", "Organic Oil"],
  fragrance: ["Unscented", "Lavender", "Rose", "Jasmine", "Sandalwood", "Vanilla", "Citrus", "Mint", "Ylang Ylang", "Chocolate", "Strawberry"],
  packSize: ["100ml", "150ml", "200ml", "250ml", "500ml", "1L"],
  brand: ["Durex", "Kamasutra", "Manforce", "Skore", "Moods", "Bold Care", "Aromatherapy"]
};

function MassageOils() {
  ...
  const [filters, setFilters] = useState({ type: [], fragrance: [], packSize: [], brand: [] });
  const [openSections, setOpenSections] = useState({ type: true, fragrance: false, packSize: false, brand: false });
  ...
  const clearAllFilters = () => {
    setFilters({ type: [], fragrance: [], packSize: [], brand: [] });
    setMaxPrice(2000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "massage oil" ||
    p.name?.toLowerCase().includes("massage oil") ||
    p.name?.toLowerCase().includes("aromatherapy oil")
  );
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm sticky top-20">
      ...
      <AccordionSection title=" Oil Type" sectionKey="type" items={FILTER_CONFIG.type} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="bg-gradient-to-r from-orange-600 to-red-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Massage Oils</h1>
      </div>
<nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
            <span className="text-lg !font-medium">/</span>
            <span className="text-lg !font-medium text-[#E4921A]">Massage Oils</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Massage Oils</h2>
