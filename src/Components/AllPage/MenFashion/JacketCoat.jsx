...
// ✅ FILTER DATA - Fabric, Combo, Price (TShirts jaisa hi)
const FILTER_CONFIG = {
  // Fabric Filter
  fabric: [
    "Acrylic", "Art Silk", "Bamboo", "Chambray", "Chanderi Cotton",
    "Chanderi Silk", "Chiffon", "Cotton", "Cotton Blend", "Cotton Cambric",
    "Cotton Linen", "Cotton Silk", "Crepe", "Denim", "Elastane",
    "Jacquard", "Jute", "Jute Cotton", "Khadi Cotton", "Linen",
    "Lycra", "Lyocell", "Net", "Nylon", "Poly Cotton", "Poly Silk",
...
function JacketCoat() {
  ...
  // ✅ ONLY JACKETS & COATS PRODUCTS
  let filteredProducts = products.filter(
    (p) => p.category === "Jackets & Coats"
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col">
      ...
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl font-bold">Jackets & Coats Collection</h1>
        <p className="mt-2 text-white/80 text-sm">Stay warm & stylish with our premium jackets</p>
      </div>
      
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7 flex-shrink-0">
        <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
        <span className="text-lg !font-medium">/</span>
        <span className="text-lg !font-medium text-[#E4921A]">Jackets & Coats</span>
      </nav>
      <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7 flex-shrink-0">Jackets & Coats</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <p className="text-gray-500 text-sm">No Jackets & Coats found matching your filters.</p>
                <button 
