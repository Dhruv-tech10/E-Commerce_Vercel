// src/component/KundanCollection.jsx
import { useState, useEffect } from "react";
import { Slider } from "@mui/material";
import ProductCard from "../ProductCard";
import { Menu, X, ChevronDown, ChevronUp } from "lucide-react";
import { weddingProducts, loadAllWeddingProducts } from "../ProductsData";
import { Link } from 'react-router-dom';
// 👑 KUNDAN COLLECTION FILTER CONFIGURATION DATA
const FILTER_CONFIG = {
  kundanTypes: ["Kundan Necklace", "Kundan Earrings", "Kundan Bangles", "Kundan Rings", "Kundan Maang Tikka"],
  sizes: ["Small", "Medium", "Large", "Adjustable"],
  brands: ["Kundan House", "Rajwada", "Jaipur Gems", "Kundan World", "Traditional Jewellery"]
};

// Sidebar configuration
const SIDEBAR_SECTIONS = [
  { title: " Kundan Types", key: "kundanTypes" },
  { title: " Sizes", key: "sizes" },
  { title: " Brands", key: "brands" }
];

function KundanCollection() {
  ...
  const [filters, setFilters] = useState({ kundanTypes: [], sizes: [], brands: [] });
  const [openSections, setOpenSections] = useState({ kundanTypes: true, sizes: true, brands: true });

  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const loadedProducts = await loadAllWeddingProducts();
        const kundanProducts = loadedProducts.filter(p => 
          p.category === "Kundan Collection" || 
          p.subCategory === "Kundan Collection" ||
          p.for === "Kundan Collection"
        );
        setProducts(kundanProducts.length > 0 ? kundanProducts : loadedProducts.slice(0, 20));
      } catch (error) {
        setProducts(weddingProducts.filter(p => p.category === "Kundan Collection"));
      } finally {
        setLoading(false);
      }
    };
  ...
  const clearAllFilters = () => {
    setFilters({ kundanTypes: [], sizes: [], brands: [] });
    setMaxPrice(5000);
  };
  ...
  const filteredProducts = products
    .filter((product) => {
      ...
      if (filters.kundanTypes.length > 0 && !filters.kundanTypes.includes(product.subCategory)) return false;
          ...
          {items.map((item, index) => (
            <label key={index} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
              <input type="checkbox" checked={filters[sectionKey]?.includes(item) || false}
                onChange={() => handleFilterToggle(sectionKey, item)} className="w-3.5 h-3.5 !mr-2 !text-sm rounded border-gray-300 text-orange-600 focus:ring-orange-500" />
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-600 mx-auto mb-3"></div><p className="text-gray-500 text-sm font-medium">Loading Kundan Collection...</p></div></div>;

  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="mx-4 my-4 !shadow-3xl shadow-red-900/30 bg-[#1E2D42] bg-gradient-to-br from-[#EE971D] to-[#1E2D42] rounded-xl text-white text-center py-12 ">
        <h1 className="text-3xl md:text-4xl font-black tracking-wide"> Kundan Collection</h1>
        <p className="mt-2 text-amber-100 text-lg">Exquisite Kundan jewellery</p>
      </div>
        <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Kundan Collection</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Kundan Collection</h2>
        ...
        <div className="flex gap-6 items-start">
          ...
          <main className="flex-1">
            <div className="bg-white rounded-xl p-3 mb-4 border border-gray-100 flex justify-between items-center shadow-sm">
              <span className="text-xs text-gray-500 font-medium">Showing {filteredProducts.length} kundan items</span>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl border p-12 text-center shadow-sm text-gray-400 font-medium text-sm">
                ...
                No kundan products found matching your criteria.
...
export default KundanCollection;