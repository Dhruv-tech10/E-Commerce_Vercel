...
const FILTER_CONFIG = {
  color: ["Maroon", "Red", "Gold", "Pink", "Orange", "Yellow", "Saffron", "White", "Cream", "Beige", "Green", "Blue", "Purple", "Brown", "Black", "Multicolor"],
  material: ["Silk", "Raw Silk", "Broken Silk", "Georgette", "Velvet", "Cotton", "Polyester", "Organza", "Net", "Chiffon", "Banarasi Silk", "Kanjeevaram"],
  type: ["Pagdi", "Safa", "Turban", "Rajasthani Pagdi", "Punjabi Pagg", "Marathi Pheta", "Designer Safa", "Wedding Safa", "Royal Safa", "Pre-Tied Safa", "Jodhpuri Safa", "Gujarati Safa"],
  size: ["Free Size", "Adjustable", "One Size", "Medium", "Large"],
  embellishment: ["Zari Work", "Embroidery", "Kundan Work", "Stone Work", "Sequin Work", "Beads Work", "Motif Work", "Border Work", "Plain", "Printed", "Brooch Attached"],
  occasion: ["Wedding", "Engagement", "Reception", "Sangeet", "Haldi", "Mehendi", "Party Wear", "Festival", "Religious Ceremony"],
  brand: ["Manyavar", "Kalki Fashion", "Utsav Fashion", "Mirraw", "CBazaar", "Indian Wedding", "Saffron Saga", "Jaypore", "Tarinika", "Kashi Trends"]
};
...
function Safa() {
  ...
  const [filters, setFilters] = useState({ color: [], material: [], type: [], size: [], embellishment: [], occasion: [], brand: [] });
  const [openSections, setOpenSections] = useState({ 
    color: true, material: true, type: true, size: false, embellishment: false, occasion: false, brand: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ color: [], material: [], type: [], size: [], embellishment: [], occasion: [], brand: [] });
    setMaxPrice(5000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "safa" ||
    p.category?.toLowerCase() === "pagdi" ||
    p.category?.toLowerCase() === "turban" ||
    p.subCategory?.toLowerCase() === "safa" ||
    p.name?.toLowerCase().includes("safa") ||
    p.name?.toLowerCase().includes("pagdi") ||
    p.name?.toLowerCase().includes("turban") ||
    p.name?.toLowerCase().includes("pagg")
  );
  ...
  filteredProducts = filteredProducts.filter((product) => {
    ...
    if (filters.embellishment.length > 0 && !filters.embellishment.includes(product.embellishment)) return false;
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      ...
      <AccordionSection title=" Embellishment" sectionKey="embellishment" items={FILTER_CONFIG.embellishment} />
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-orange-600 to-red-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Safa & Pagdi Collection</h1>
      </div>
 <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Safa & Pagdi</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Safa & Pagdi</h2>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Safas Found</h3>
