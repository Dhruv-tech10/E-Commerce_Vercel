...
const FILTER_CONFIG = {
  color: ["Gold", "Yellow Gold", "Rose Gold", "White Gold", "Silver", "Black", "Brown", "Multicolor", "Red", "Green", "Blue"],
  material: ["Gold", "Silver", "Alloy", "Steel", "Brass", "Copper", "Kundan", "Artificial Jewellery", "Tarnish Resistant", "Nickel Free"],
  type: ["Payal", "Anklet", "Wedding Payal", "Bridal Anklet", "Designer Payal", "Traditional Payal", "Silver Payal", "Gold Payal", "Kundan Payal", "Stone Payal", "Pearl Payal", "Lightweight Payal", "Heavy Payal"],
  length: ["8 inch", "9 inch", "10 inch", "11 inch", "12 inch", "13 inch", "14 inch", "15 inch", "Adjustable", "One Size"],
  closureType: ["Hook", "Lobster Clasp", "Magnetic", "Adjustable Chain", "Elastic", "Slip On", "Screw Type"],
  occasion: ["Wedding", "Engagement", "Reception", "Sangeet", "Mehendi", "Haldi", "Party Wear", "Festival", "Casual", "Daily Wear", "Traditional Event"],
  workType: ["Kundan Work", "Stone Work", "Pearl Work", "Beads Work", "Zari Work", "Filigree Work", "Enamel Work", "Meenakari Work", "Plain", "Matte Finish", "Shiny Finish", "Antique Finish"],
  brand: ["Tanishq", "Malabar Gold", "Kalyan Jewellers", "PC Jeweller", "Caratlane", "Bluestone", "Melorra", "GIVA", "Voylla", "Sia Art Jewellery", "Miraggio", "Zaveri Pearls", "Pipa Bella", "Ayesha Accessories"]
};
...
function Payal() {
  ...
  const [filters, setFilters] = useState({ color: [], material: [], type: [], length: [], closureType: [], occasion: [], workType: [], brand: [] });
  const [openSections, setOpenSections] = useState({ 
    color: true, material: true, type: true, length: false, closureType: false, occasion: false, workType: false, brand: false
  });
  ...
  const clearAllFilters = () => {
    setFilters({ color: [], material: [], type: [], length: [], closureType: [], occasion: [], workType: [], brand: [] });
    setMaxPrice(10000);
    setSortBy("default");
  };

  let filteredProducts = products.filter(p => 
    p.category?.toLowerCase() === "payal" ||
    p.category?.toLowerCase() === "anklet" ||
    p.subCategory?.toLowerCase() === "payal" ||
  ...
  const AccordionSection = ({ title, sectionKey, items }) => (
    <div className="mb-4 border-b border-gray-100 pb-3">
      <button onClick={() => toggleSection(sectionKey)} className="flex justify-between w-full font-bold text-gray-800 text-sm mb-2 hover:text-gold-600 transition">
        <span>{title}</span>
  ...
  const FilterSidebar = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm">
      <div className="flex justify-between items-center mb-4 border-b pb-3">
        ...
        <button onClick={clearAllFilters} className="text-amber-600 text-xs hover:underline">Clear All</button>
      </div>
      ...
      <div className="mb-4 pt-2">
        ...
        <Slider value={maxPrice} onChange={(e, val) => setMaxPrice(val)} min={0} max={15000} step={200}
          sx={{ color: "#d97706", height: 4, '& .MuiSlider-thumb': { width: 18, height: 18, backgroundColor: '#d97706' } }} />
      </div>
    </div>
  );

  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-600"></div></div>;

  return (
    <div className="bg-gray-100 min-h-screen">
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-amber-600 to-yellow-600 text-white text-center py-12">
        <h1 className="text-4xl font-bold">Payal & Anklet Collection</h1>
      </div>

       <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Payal</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Payal</h2>
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-black/60" onClick={() => setMobileFiltersOpen(false)}>
          <div className="absolute top-0 left-0 w-[85%] h-full bg-white overflow-y-auto p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sticky bottom-0 bg-white pt-4 pb-2 mt-4 border-t">
              <button onClick={() => setMobileFiltersOpen(false)} className="w-full bg-amber-600 text-white py-3 rounded-lg font-semibold">
                Apply Filters
              </button>
            </div>
          </div>
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">No Payals Found</h3>
                <p className="text-gray-500 mb-4">Try adjusting your filters or check back later</p>
                <button onClick={clearAllFilters} className="bg-amber-600 text-white px-6 py-2 rounded-lg hover:bg-amber-700 transition">
                  Clear All Filters
                </button>
              </div>
            ) : (
