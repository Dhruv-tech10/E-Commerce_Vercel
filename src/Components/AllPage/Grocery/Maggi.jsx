...
function Maggi() {
  ...
  const getFilteredProducts = () => {
    let filtered = products.filter((p) => 
      p.category === "Maggi" || 
      p.category === "Grocery" ||
      p.category?.toLowerCase().includes("maggi") ||
      p.type?.toLowerCase().includes("veg")
    );

    // Only show vegetarian products
    ...
    if (activeFilters.brand.length > 0) {
      filtered = filtered.filter((product) =>
        activeFilters.brand.some(
          (brand) => product.brand?.toLowerCase().includes(brand.toLowerCase().replace(" (veg)", ""))
        )
      );
    }
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
        ...
        <h1 className="text-3xl font-bold">Maggi</h1>
        <p className="mt-2 text-white/80 text-sm">Timeless Elegance for Every Occasion</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Maggi</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Maggi</h2>
              ...
              {openDropdown.price && (
                <div className="mt-4 px-1">
                  ...
                  <div className="relative w-full h-1 bg-gray-300 rounded-full mt-1">
                    <div className="absolute h-1 bg-orange-500 rounded-full" style={{ width: `${(maxPrice / 200) * 100}%` }} />
                    <input
                      type="range"
                      min="0"
                      max="200"
                    ...
                    />
                    <div className="absolute top-1/2 -mt-3 w-6 h-6 bg-orange-500 rounded-full shadow-md z-5 pointer-events-none" style={{ left: `calc(${(maxPrice / 200) * 100}% - 12px)` }} />
                  </div>
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-96 text-center">
              ...
              <p className="text-gray-400 mb-4 max-w-md">
                {hasActiveFilters ? "We couldn't find any vegetarian Maggi products matching your selected filters." : "No vegetarian Maggi products are available at the moment."}
              </p>
              {hasActiveFilters && (
