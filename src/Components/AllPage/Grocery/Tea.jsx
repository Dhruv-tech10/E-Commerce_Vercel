...
function Tea() {
  ...
  const getFilteredTea = () => {
    let filtered = products.filter((p) => p.category === "Tea" || p.category === "Grocery");
  ...
  const filteredTeaProducts = getFilteredTea();
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
            ...
            <span className="text-lg !font-medium text-[#E4921A]">Tea</span>
          </nav>
          <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Tea</h2>  
              ...
              {openDropdown.price && (
                <div className="mt-4 px-1">
                  ...
                  <div className="relative w-full h-1 bg-gray-300 rounded-full mt-1">
                    ...
                    {/* Custom Orange Handle */}
                    <div 
                      className="absolute top-1/2 -mt-3 w-6 h-6 bg-orange-500 rounded-full shadow-md z-5 pointer-events-none"
                      style={{ 
                        left: `calc(${(maxPrice / 500) * 100}% - 12px)`,
          ...
          <h1 className="text-3xl font-bold mb-6">
            ...
            {hasActiveFilters && (
              <span className="text-lg font-normal text-gray-500 ml-2">
                ({filteredTeaProducts.length} products)
              </span>
            )}
          </h1>

          {filteredTeaProducts.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {filteredTeaProducts.map((product) => (
            ...
            <div className="flex flex-col items-center justify-center h-96 text-center">
              ...
              <p className="text-gray-400 mb-4 max-w-md">
                {hasActiveFilters
                  ? "We couldn't find any tea products matching your selected filters. Please try different filter combinations."
                  : "No tea products are available at the moment. Please check back later."}
              </p>
              {hasActiveFilters && (
                <button
                  onClick={clearNewFilters}
