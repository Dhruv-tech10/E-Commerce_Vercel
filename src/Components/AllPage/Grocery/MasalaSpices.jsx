...
function MasalaSpices() {
  ...
  const getFilteredProducts = () => {
    let filtered = products.filter((p) => p.category === "Masala Spices" || p.category === "Grocery");
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
        ...
        <h1 className="text-3xl font-bold">Masala Spices</h1>
        <p className="mt-2 text-white/80 text-sm">Timeless Elegance for Every Occasion</p>
      </div>
      <nav className="flex items-center gap-2 text-sm text-gray-500 mt-3 !pl-7">
          <Link to="/" className="!text-lg !no-underline !font-semibold !text-[#1E2D42] transition-colors">Home</Link>
          <span className="text-lg !font-medium">/</span>
          <span className="text-lg !font-medium text-[#E4921A]">Masala Spices</span>
        </nav>
        <h2 className="text-2xl !font-bold !mt-3 !mb-3 !pl-7">Masala Spices</h2>
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-96 text-center">
              ...
              <p className="text-gray-400 mb-4 max-w-md">
                {hasActiveFilters ? "We couldn't find any products matching your selected filters." : "No masala products are available at the moment."}
              </p>
              {hasActiveFilters && (
